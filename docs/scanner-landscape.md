...
The latest comparator gap is Next.js
[GHSA-492v-c6pp-mqqv](https://github.com/vercel/next.js/security/advisories/GHSA-492v-c6pp-mqqv).
The package issue becomes an application authorization path only when a
middleware/proxy visible-path gate protects a dynamic route, the same segment
enters server-side data access, route-local authorization is absent, and an
affected deployment honors an external `nxtP<segment>` internal route value.
The [repair](https://github.com/vercel/next.js/commit/87080764c96f5416decccd43f4c434545fd5d4e1)
distinguishes trusted proxy normalization from ordinary requests already
wrapped by the normal Next server. Current authenticated CodeQL and Semgrep
rule-source searches contain neither the advisory nor an exact `nxtPslug`
model. Copilot Security therefore records the source/gate/route/data/provenance
chain and requires deployment-specific validation; it does not treat every
affected package or ordinary `next start` process as exploitable.

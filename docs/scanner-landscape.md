...
| [OWASP API1:2023 Broken Object Level Authorization](https://owasp.org/API-Security/editions/2023/en/0xa1-broken-object-level-authorization/), [Go `database/sql`](https://pkg.go.dev/database/sql), [CodeQL Go query index](https://codeql.github.com/codeql-query-help/go/), and [gosec rules](https://github.com/securego/gosec/blob/master/RULES.md) | OWASP requires authorization for every endpoint that receives an object ID and acts on the selected object. The current CodeQL Go index and published gosec rule list do not expose a dedicated object-level authorization query. Go's standard library makes the fixed query, placeholder arguments, `QueryRow`/`Scan`, and mutation dispatch observable. | Require an exact typed request-to-object-predicate path and actual read disclosure or mutation; retain same-query security predicates only when bound to a context-derived principal; retain fail-closed returned-owner checks only before the protected effect; and reject authentication-only, attacker-controlled owner filters, opaque IDs, parameterization-only, lookalike APIs, and generic responses. |
...
key. The matched control adds an account predicate whose value comes from the
authenticated request context; its witness proves the cross-account read is
not found while the attacker's own invoice remains available. The typed host
requires exact `net/http` and `database/sql` identities, object-key placeholder
roles, DB/Tx/Conn receivers, and either read-to-response closure or an executed
...
as a dedicated query. Cross-package and multi-hop wrappers, Query/Rows loops,
joins, composite keys, row mappers, policy services, sqlc, ORMs, branch-

...
| [OWASP API1:2023 Broken Object Level Authorization](https://owasp.org/API-Security/editions/2023/en/0xa1-broken-object-level-authorization/), [Go `database/sql`](https://pkg.go.dev/database/sql), [CodeQL Go data flow](https://codeql.github.com/docs/codeql-language-guides/analyzing-data-flow-in-go/), [CodeQL Go library](https://codeql.github.com/docs/codeql-language-guides/codeql-library-for-go/), [Semgrep analysis scope](https://semgrep.dev/docs/writing-rules/glossary), and [gosec rules](https://github.com/securego/gosec/blob/master/RULES.md) | OWASP requires authorization for every endpoint that receives an object ID and acts on the selected object; its shop-revenue listing example is explicitly collection-shaped. CodeQL distinguishes local flow from configurable global interprocedural flow; Semgrep documents per-file Community Edition and cross-file/cross-function proprietary analysis. The current CodeQL Go query index and published gosec rule list do not expose a dedicated object-level authorization query. Go's standard library makes fixed query arguments, read, mutation, prepared-statement, and transaction lifecycles observable. | Require an exact typed request-to-object-predicate path and actual protected disclosure or durable mutation. Compose bounded unique same-package and local-module wrapper paths with explicit call, parameter, alias, and principal evidence; retain same-query security predicates only for the exact context-derived principal; and reject authentication-only, ambiguous or over-depth graphs, unrelated object state, opaque IDs, parameterization-only, lookalike APIs, and generic responses. |
...
succeeds. This directly implements OWASP API1:2023's
...
failure semantics and executable object/principal controls. Methods,
interfaces, receiver fields, function-value object wrappers, nested and
...
1. **Expand typed framework security models.** Extend bounded summaries beyond
   ...
   methods, interfaces, receiver fields, policy services, sqlc, and ORMs;

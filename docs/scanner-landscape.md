...
and Tokio process builders. Copilot Security already distinguished executable
selection and real shell, interpreter, Windows batch, and raw command-line
grammar from ordinary argv, and required actual process dispatch. It now
preserves that higher-precision boundary for exact `tokio::process::Command`
imports and aliases. This closes the measured Axum/Tokio false negative without
copying CodeQL's ordinary-`grep`-argument alert. Current Tokio `status()` and
`output()` spawn before returning their Future, so the model correctly does not
treat missing `await` as process-creation prevention. The strict Tokio 1.53.1
pair retains a fixed-`printf` literal argument as its negative control.

The preceding comparator adoption was Semgrep's open August 2026
[`asyncpg-sqli` taint-mode repair](https://github.com/semgrep/semgrep-rules/pull/4014),
which demonstrates an alias-flow false negative after a query is copied into a
second local. Copilot Security now follows that copy but narrows the result to
actual remote sources, official live `asyncpg.Connection` or compatible
`Pool` receivers, the grammar-bearing query/command argument, and `await` or
async cursor consumption. It also covers the official underscored
`copy_from_query` name and current `fetchmany`, which the proposal misses, and
rejects later protocol-bound values, unawaited calls, invalid Pool methods,
lookalikes, shadows, reassignment, and malformed signatures. The strict pair
keeps fixed `$1` SQL as its negative control, while reporting still requires
PostgreSQL behavior, role, authorization, and concrete impact.

The preceding comparator adoption was CodeQL's August 2026
[`R2DBC SQL injection models`](https://github.com/github/codeql/pull/22279).
Copilot Security now recognizes both an exact official Spring `DatabaseClient`
and the lower-level `io.r2dbc.spi` `Connection`, `Statement`, and `Batch`
grammar boundaries. It keeps SQL or identifier grammar distinct from `bind`
values and requires a documented reactive execution transition. It goes beyond
a sink signature by distinguishing `Batch.add(String)` from parameterless
`Statement.add()`, following assigned and fluent receivers, retaining multiple
sinks on one line, and rejecting inert operations, local or competing types,
wrong arity, and reassignment. Reviewer proof still closes Result consumption,
driver/dialect behavior, database authority, and concrete impact. Separate
Spring and SPI H2 pairs gate predicate-changing interpolation against the same
input used only as a bound value.

The preceding comparator adoption was CodeQL's July 2026
...
| --------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- | ------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- | ---------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- |
| [CodeQL Rust command-injection query](https://github.com/github/codeql/pull/22323), its [`Command` sink map](https://github.com/github/codeql/blob/main/rust/ql/lib/codeql/rust/frameworks/stdlib/process.model.yml), and the official [Tokio 1.53.1 process implementation](https://github.com/tokio-rs/tokio/blob/tokio-1.53.1/tokio/src/process/mod.rs)                                                                                                                                                                                                                                                                                                                                                                              | CodeQL adds active-threat sources, numeric/boolean/enum barriers, allowlist guards, and matching `new`/`arg`/`args` sinks for `std` and Tokio. Its broad query test alerts on an ordinary remote argument to fixed `grep`, despite its help recommending separate arguments. Tokio additionally calls `spawn()` before returning the `status()` or `output()` Future.                                                                                                                                                                                                                                                                             | Preserve exact `std`/Tokio receiver identity through the mutable builder and require actual dispatch. Distinguish executable selection and explicit shell/interpreter/batch/raw grammar from literal argv, retain the latter as a negative control, and do not mistake missing `await` for prevention of Tokio process creation. Reject local Tokio modules, foreign crate aliases, unsupported Tokio `exec`, inert builders, and replaced state; validate child lifetime, output handling, OS, privileges, containment, and only the demonstrated effect.                                                                                                                                                                                                                                                             |
| [Semgrep asyncpg taint-rule repair](https://github.com/semgrep/semgrep-rules/pull/4014) and the official [asyncpg Connection API](https://magicstack.github.io/asyncpg/current/api/index.html#connection)                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                               | Taint mode repairs copied-query false negatives, while exact API signatures distinguish the grammar-bearing query/command from later protocol-bound values. Async calls also separate coroutine creation from execution.                                                                                                                                                                                                                                                                                                                                                                                                                          | Require a live official Connection or compatible Pool origin, exact query role, and await/cursor consumption; follow local copies and relative wrappers. Cover correct `copy_from_query` and current `fetchmany`, reject invalid Pool methods, later bound values, inert calls, lookalikes, shadows, reassignment, and malformed signatures, then require PostgreSQL-specific behavior and impact evidence.                                                                                                                                                                                                                                                                                                                                                                                                            |
| [CodeQL R2DBC models](https://github.com/github/codeql/pull/22279), [Spring R2DBC `DatabaseClient`](https://docs.spring.io/spring-framework/reference/data-access/r2dbc.html), [R2DBC `Connection`](https://javadoc.io/static/io.r2dbc/r2dbc-spi/1.0.0.RELEASE/io/r2dbc/spi/Connection.html), [`Statement`](https://r2dbc.io/spec/1.0.0.RELEASE/api/io/r2dbc/spi/Statement.html), [`Batch`](https://r2dbc.io/spec/1.0.0.RELEASE/api/io/r2dbc/spi/Batch.html), and [statement execution](https://r2dbc.io/spec/1.0.0.RELEASE/spec/html/#statements)                                                                                                                                                                                      | Spring separates SQL text or a Supplier from bound values and moves a `DatabaseClient` specification into execution through `fetch`, `then`, or mapping methods. The SPI separately exposes statement and batch SQL, savepoint identifiers, generated-column identifiers, and reactive execution; `Statement.add()` itself accepts no SQL.                                                                                                                                                                                                                                                                                                        | Require exact official receiver identity and exact grammar positions; retain fluent or assigned execution and returned/subscribed Publisher transitions across typed Java wrappers. Preserve multiple fluent boundaries on one line and constructor-initialized final fields, but reject out-of-scope parameter shadows, fixed SQL plus `bind`, inert operations, lookalikes, wrong arity, and reassignment. Separately validate Result consumption, driver/dialect quoting, privileges, tenancy, and concrete effect.                                                                                                                                                                                                                                                                                                 |
...
SQL wrappers, exact FastAPI/Pydantic request-body fields reaching shell grammar,
and FastAPI string query parameters reaching an HTTP redirect location. The
negative cases preserve fixed shell-free execution, multiline DB-API parameter
binding, `ClassVar` exclusion from request data, and a hostile absolute URL
encoded beneath a fixed local redirect path. Deterministic tests also reject
fixed arguments, reassignment after request input, ambiguous absolute imports,
comments, string-only pseudo-calls, configured or ambiguous Query metadata,
local FastAPI or Starlette shadows, and replaced redirect bindings. The SQL and
open-redirect expectations accept medium severity because their fixtures prove
unauthorized row selection or origin selection but deliberately do not invent

...
| [CodeQL Java SSRF](https://codeql.github.com/codeql-query-help/java/java-ssrf/), [Spring WebClient](https://docs.spring.io/spring-framework/reference/web/webflux-webclient.html), and [JDK redirect policy](https://docs.oracle.com/en/java/javase/21/docs/api/java.net.http/java/net/http/HttpClient.Redirect.html) | High-precision Java SSRF analysis tracks request data into outbound clients; WebClient delegates transport behavior to its configured connector; server-owned authorized URLs are preferred, and redirect policy applies only after the initial request. | Track Spring/servlet values across typed Java services into JDK `HttpClient`, `RestTemplate`, and reactive `WebClient`, while keeping fixed complete destination selection, connector redirect rejection, and address pinning as distinct proof obligations. |
...
`RestTemplate`, or reactive `WebClient.UriSpec.uri` sinks. The WebClient model
...
1. **Expand typed framework security models.** Extend bounded summaries beyond
   ...
   Spring `WebClient`, and .NET `HttpClient`, partial-URL SSRF models,

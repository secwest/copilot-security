...
The latest comparator gap is Pickem
...
| [Ktor type-safe routing](https://ktor.io/docs/server-resources.html), [JDK `ProcessBuilder`](https://docs.oracle.com/en/java/javase/26/docs/api/java.base/java/lang/ProcessBuilder.html), and [CodeQL Java/Kotlin command-line injection](https://codeql.github.com/codeql-query-help/java/java-command-line-injection/)                                                                                                                                                                                                                                                                                                                                                                                                                | Ktor Resources deserializes path and query data into an annotated handler value. `ProcessBuilder` keeps an ordered command that can be replaced after construction. A list passed to the constructor or `command(List)`, and the list returned by `command()`, remains live rather than being copied; ordered mutations therefore affect later `start` and `startPipeline` execution. A program-and-argument list does not imply a shell.                                                                                                                                                                                                         | Bind only exact Ktor call or typed-resource sources and exact JDK builder identities; follow shared command-list identity, builder and list aliases, replacement, indexed writes, `set`, `add`, `removeAt`, and clear/rebuild order into actual execution; detach old views after replacement; distinguish executable selection and explicit command-language positions from ordinary argv; and preserve annotation, route import, builder identity, mutation, reassignment, and execution as proof obligations.                                                                                       |

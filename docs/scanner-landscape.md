...
The latest comparator gap is DeepSeek MCP
[GHSA-fh3r-g96v-f578](https://github.com/arikusi/deepseek-mcp-server/security/advisories/GHSA-fh3r-g96v-f578).
The advisory distinguishes an HTTP multi-client authorization defect from the
safe stdio deployment: affected releases create separate MCP servers per HTTP
session but route their tools through one process-global caller-keyed session
store. The [1.7.0 repair](https://github.com/arikusi/deepseek-mcp-server/commit/9fd514292d23d59ca1434b01f019aca6ef4356f9)
injects a fresh store into every HTTP session. Current authenticated CodeQL and
Semgrep rule-source searches have no exact package match. Copilot Security
therefore binds production package proof, literal HTTP selection, and a real
top-level or operational-script launch, with stdio, inactive launchers,
ambiguous metadata, and 1.7.0 retained as explicit counterexamples.

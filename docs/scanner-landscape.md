...
The latest comparator gap is Plate media
[GHSA-qj6x-xx2h-8hvv](https://github.com/udecode/plate/security/advisories/GHSA-qj6x-xx2h-8hvv).
Package presence becomes a browser XSS path only when remote serialized editor
state enters a configured Plate value, the application registers a media
component, official `useMediaState` trusts attacker-supplied provider metadata,
and the resulting `embed.url` reaches a script-capable iframe. The
[repair](https://github.com/udecode/plate/commit/6214914ca811adf22d0ad503154494216eed68ba)
removes the metadata fast path and recomputes embed state from the render URL.
Current authenticated CodeQL and Semgrep rule-source searches contain neither
the package nor the hook. Copilot Security therefore records the complete
document/prop/editor/plugin/hook/iframe/provenance chain and requires a real
browser validation before claiming script execution or application impact.

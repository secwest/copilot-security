...
| [CodeQL Go command injection](https://codeql.github.com/codeql-query-help/go/go-command-injection/), [Go `os/exec`](https://pkg.go.dev/os/exec), [CodeQL's `os/exec` model](https://github.com/github/codeql/blob/main/go/ql/lib/ext/os.exec.model.yml), and [gosec G204](https://github.com/securego/gosec/blob/master/rules/subproc.go)                                                                                                                                                                                   | Go constructs an argument vector rather than invoking a shell automatically, but executable names, explicit shell/interpreter command strings, script paths, remote commands, and option-sensitive tools can still provide execution. CodeQL tracks executable and indirect shell-like arguments with command-specific `--` behavior. Gosec G204 flags unresolved `Command`, `CommandContext`, execabs, and syscall arguments at construction time.                                                                                                                                             | Require exact imports and typed request sources; distinguish executable, command-string, script, remote-command, and ordinary data arguments; preserve immutable command and `--` barriers; and require the same constructed `Cmd` to reach `Run`, `Start`, `Output`, or `CombinedOutput` before reporting.                                                           |
...
process-execution lane. The positive carries an indexed HTTP value through one
unique same-package wrapper, formats it into the command string following
`sh -c`, and closes construction through `CombinedOutput`. The matched control
retains the request, wrapper, exact `os/exec` import, shell, flag, execution
method, and attack bytes but uses the value only to select a complete immutable
server-owned command. Both modules copy their running Go test executable to a
temporary `sh` path and activate a bounded command witness, so the exploit and
control execute deterministically on Windows and Linux without trusting or
invoking the host shell.
...
`Command` or `CommandContext` is retained only until the same non-reassigned
command reaches `Run`, `Start`, `Output`, or `CombinedOutput`; construction,
pipe setup, `LookPath`, and unrelated same-named methods are inert. Exact
aliases and execabs are supported, while forks, import lookalikes, dot or
...
adds explicit construction-to-execution closure, and avoids gosec G204's broad
construction-time treatment of unresolved arguments. Remaining work includes
manual `exec.Cmd` composites and field mutation, syscall process APIs, nested
`env`/`sudo`/`timeout` command selection, variadic slice assembly, cross-package
and multiple wrapper hops, PATH/environment mutation, working-directory and
credential inheritance, and platform-specific deployment evidence.

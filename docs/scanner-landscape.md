...
The latest comparator gap is Defuddle
[GHSA-jg4p-g6xj-4qmf](https://github.com/kepano/defuddle/security/advisories/GHSA-jg4p-g6xj-4qmf).
Package presence and parsing are not yet an application XSS path: remote HTML
must reach an affected site extractor, the returned `content` must remain
unsanitized, and a downstream consumer must render it as HTML. The
[repair](https://github.com/kepano/defuddle/commit/baf2eaef61d334ef595b28c89e5c5e89e52daf7f)
escapes the confirmed interpolation sinks and sends every extractor result
through the central DOM sanitizer. Current authenticated CodeQL and Semgrep
rule-source searches contain no Defuddle, advisory, or CVE match. Copilot
Security therefore records the complete remote-source/wrapper/parser/returned-
property/render-boundary/provenance chain and uses a non-executing real-package
differential before any downstream browser or application-impact claim.

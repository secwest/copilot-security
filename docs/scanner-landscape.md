...
| [GORM v2 security](https://gorm.io/docs/security.html), [`gorm.io/gorm` API](https://pkg.go.dev/gorm.io/gorm), and [CodeQL's GORM model](https://github.com/github/codeql/blob/main/go/ql/lib/ext/gorm.io.gorm.model.yml)                                                                                                                                                                                                                                                                                                   | GORM safely binds later placeholder values but accepts raw SQL or structural fragments in `Raw`, `Exec`, `Where`, `Order`, `Table`, joins, inline conditions, `Pluck`, and `gorm.Expr`. Most chain methods build a statement; finishers perform query or mutation callbacks. CodeQL lists exact GORM fragment arguments, but its visible extension model attaches sinks to construction calls rather than expressing builder-to-finisher closure. | Prove exact v2 import and `*gorm.DB` identity; preserve grammar versus value positions and fluent object identity; require deferred fragments to reach a finisher; model `gorm.Expr` grammar; and retain DryRun, dialect, fragment allowlists, privileges, tenancy, and concrete impact separately. |
...
lane. The positive carries an HTTP query value through one unique same-package
wrapper, formats it into SQL, and retains that grammar across `Raw` until
`Scan` executes it. The matched control preserves the handler, wrapper,
`*gorm.DB`, attack bytes, and deterministic driver but keeps query text fixed
and supplies the bytes only as a placeholder value. Exact local replacement
modules provide a minimal signature-compatible `gorm.io/gorm` subset, so both
behaviors execute offline without claiming to reproduce all ORM semantics.
...
comment/string examples. This adds explicit construction-to-finisher closure
to the argument positions in [CodeQL's visible GORM model](https://github.com/github/codeql/blob/main/go/ql/lib/ext/gorm.io.gorm.model.yml)
and follows GORM's official distinction between [safe placeholders and
injectable SQL fragments](https://gorm.io/docs/security.html). Generics API
chains, cross-package and multi-hop wrappers, custom clauses, plugin callbacks,
branch-sensitive DryRun dominance, other ORMs and builders, dialect metadata,
and deployment database-role evidence remain future work.

...
The versioned corpus currently contains twenty-one vulnerable/control pairs:
...
disabled TLS certificate verification, predictable security tokens, server-
side template injection, check/use state races, unsafe mass assignment, cookie-
authenticated cross-site request forgery, attacker-length native-memory
corruption, and adversarial repository instructions that try to suppress a real
finding while inducing a false one. Three runs per case produce 126 scans in the
complete corpus.

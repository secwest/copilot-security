...
An Express JSON object crosses three relative-import wrappers into a source operand of `lodash.merge()`. The nearest runtime manifest pins Lodash 4.17.11, before the 4.17.12 prototype-pollution fix, so a `constructor.prototype` path can modify `Object.prototype` and supply inherited authorization state.

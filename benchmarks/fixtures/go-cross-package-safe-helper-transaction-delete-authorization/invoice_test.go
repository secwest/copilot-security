...
func TestPrincipalPredicateScopesCrossPackageCommittedDeletion(t *testing.T) {
	state := &invoiceState{owners: map[string]string{
		"victim-invoice":   "victim-account",
		"attacker-invoice": "attacker-account",
	}}
	sql.Register("authorized-cross-package-transaction-witness", crossPackageTransactionDriver{state: state})
	db, err := sql.Open("authorized-cross-package-transaction-witness", "")
	if err != nil {
		t.Fatal(err)
	}
	defer db.Close()
	...
	if state.owners["victim-invoice"] != "victim-account" {
		t.Fatal("cross-account cross-package deletion became durable")
	}
	...
	if _, exists := state.owners["attacker-invoice"]; exists {
		t.Fatal("owned cross-package deletion did not become durable")
	}
}

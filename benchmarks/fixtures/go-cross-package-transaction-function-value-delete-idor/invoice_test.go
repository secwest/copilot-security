...
func TestAttackerCommitsVictimDeletionThroughFunctionValues(t *testing.T) {
	state := &invoiceState{owners: map[string]string{
		"victim-invoice": "victim-account",
	}}
	sql.Register("transaction-function-value-delete-idor-witness", functionValueTransactionDriver{state: state})
	db, err := sql.Open("transaction-function-value-delete-idor-witness", "")
	if err != nil {
		t.Fatal(err)
	}
	defer db.Close()
	...
	if _, exists := state.owners["victim-invoice"]; exists {
		t.Fatal("function-value victim deletion did not become durable")
	}
}

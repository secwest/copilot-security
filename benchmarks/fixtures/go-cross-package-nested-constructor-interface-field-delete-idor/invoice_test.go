...
func (connection *witnessConn) ExecContext(_ context.Context, query string, arguments []driver.NamedValue) (driver.Result, error) {
	if query != "DELETE FROM invoices WHERE id = ?" || len(arguments) != 1 {
		return nil, errors.New("unexpected nested-constructor mutation")
	}
	*connection.deleted = true
	return driver.RowsAffected(1), nil
}

func TestAttackerDeletesVictimThroughNestedConstructorFields(t *testing.T) {
	deleted := false
	sql.Register("nested-constructor-field-idor", witnessDriver{deleted: &deleted})
	db, err := sql.Open("nested-constructor-field-idor", "")
	if err != nil {
		t.Fatal(err)
	}
	defer db.Close()
	...
	if response.Code != 204 || !deleted {
		t.Fatalf("nested constructor path did not delete: code=%d deleted=%v", response.Code, deleted)
	}
}

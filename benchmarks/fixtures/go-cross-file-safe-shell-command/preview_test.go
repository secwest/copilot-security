...
func TestFixedSelectionRejectsShellGrammar(t *testing.T) {
	...
	if strings.Contains(attackResponse.Body.String(), "internal-signing-key") {
		t.Fatalf("fixed selection exposed the internal record: %q", attackResponse.Body.String())
	}
	...
	if !strings.Contains(allowedResponse.Body.String(), "public-format") {
		t.Fatalf("safe command did not return its intended output: %q", allowedResponse.Body.String())
	}
}

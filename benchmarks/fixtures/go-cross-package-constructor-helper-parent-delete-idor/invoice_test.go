...
func TestAttackerDeletesVictimThroughConstructorParentHelper(t *testing.T) {
	deleted := false
	sql.Register("constructor-helper-parent-idor", witnessDriver{deleted: &deleted})
	db, err := sql.Open("constructor-helper-parent-idor", "")
	if err != nil {
		t.Fatal(err)
	}
	defer db.Close()
	...
	if response.Code != 204 || !deleted {
		t.Fatalf("constructor parent helper did not delete: code=%d deleted=%v", response.Code, deleted)
	}
}

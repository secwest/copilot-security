...
func TestAttackerDeletesVictimThroughHelperValueCopyPointer(t *testing.T) {
	deleted := false
	sql.Register("helper-value-copy-pointer-idor", witnessDriver{deleted: &deleted})
	db, err := sql.Open("helper-value-copy-pointer-idor", "")
	if err != nil {
		t.Fatal(err)
	}
	defer db.Close()
	...
	if response.Code != 204 || !deleted {
		t.Fatalf("helper value-copy pointer did not delete: code=%d deleted=%v", response.Code, deleted)
	}
}

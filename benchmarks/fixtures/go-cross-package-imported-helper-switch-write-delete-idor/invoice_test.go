...
func TestAttackerDeletesVictimThroughSwitchHelper(t *testing.T) {
	deleted := false
	sql.Register("switch-helper-idor", witnessDriver{deleted: &deleted})
	db, err := sql.Open("switch-helper-idor", "")
	if err != nil {
		t.Fatal(err)
	}
	defer db.Close()
	...
	if response.Code != 204 || !deleted {
		t.Fatalf("switch helper did not delete: code=%d deleted=%v", response.Code, deleted)
	}
}

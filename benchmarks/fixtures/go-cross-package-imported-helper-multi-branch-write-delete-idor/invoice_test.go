...
func TestAttackerDeletesVictimThroughMultiBranchHelper(t *testing.T) {
	deleted := false
	sql.Register("multi-branch-helper-idor", witnessDriver{deleted: &deleted})
	db, err := sql.Open("multi-branch-helper-idor", "")
	if err != nil {
		t.Fatal(err)
	}
	defer db.Close()
	...
	if response.Code != 204 || !deleted {
		t.Fatalf("multi-branch helper did not delete: code=%d deleted=%v", response.Code, deleted)
	}
}

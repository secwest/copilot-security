...
func TestAttackerDeletesVictimThroughImportedConstructorParentHelper(t *testing.T) {
	deleted := false
	sql.Register("imported-constructor-helper-parent-idor", witnessDriver{deleted: &deleted})
	db, err := sql.Open("imported-constructor-helper-parent-idor", "")
	if err != nil {
		t.Fatal(err)
	}
	defer db.Close()
	...
	if response.Code != 204 || !deleted {
		t.Fatalf("imported constructor parent helper did not delete: code=%d deleted=%v", response.Code, deleted)
	}
}

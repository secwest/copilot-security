...
func (connection *witnessConn) ExecContext(_ context.Context, query string, arguments []driver.NamedValue) (driver.Result, error) {
	if query != "DELETE FROM invoices WHERE id = ? AND account_id = ?" || len(arguments) != 2 {
		return nil, errors.New("unexpected embedded-interface control mutation")
	}
...
func TestEmbeddedInterfacesPreserveAuthorization(t *testing.T) {
	victimDeleted := false
	ownedDeleted := false
	sql.Register("embedded-interface-safe", witnessDriver{victimDeleted: &victimDeleted, ownedDeleted: &ownedDeleted})
	db, err := sql.Open("embedded-interface-safe", "")
	if err != nil {
		t.Fatal(err)
	}
	defer db.Close()
	...
	if response.Code != 204 || victimDeleted {
		t.Fatalf("embedded-interface path crossed ownership: code=%d victimDeleted=%v", response.Code, victimDeleted)
	}

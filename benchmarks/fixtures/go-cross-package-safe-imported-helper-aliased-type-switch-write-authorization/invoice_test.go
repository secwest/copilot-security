...
func TestAliasedTypeSwitchHelperPreservesAuthorization(t *testing.T) {
	victimDeleted := false
	ownedDeleted := false
	sql.Register("aliased-type-switch-helper-safe", witnessDriver{victimDeleted: &victimDeleted, ownedDeleted: &ownedDeleted})
	db, err := sql.Open("aliased-type-switch-helper-safe", "")
	if err != nil {
		t.Fatal(err)
	}
	defer db.Close()
	...
	if response.Code != 204 || victimDeleted {
		t.Fatalf("aliased type switch helper crossed ownership: code=%d victimDeleted=%v", response.Code, victimDeleted)
	}

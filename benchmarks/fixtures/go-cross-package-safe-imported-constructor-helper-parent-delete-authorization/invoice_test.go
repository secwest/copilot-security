...
func (driver_ witnessDriver) Open(string) (driver.Conn, error) {
	return &witnessConn{
		victimDeleted: driver_.victimDeleted,
		ownedDeleted:  driver_.ownedDeleted,
	}, nil
}
...
func (connection *witnessConn) ExecContext(_ context.Context, query string, arguments []driver.NamedValue) (driver.Result, error) {
	if query != "DELETE FROM invoices WHERE id = ? AND account_id = ?" || len(arguments) != 2 {
		return nil, errors.New("unexpected imported-constructor-helper mutation")
	}
...
func TestImportedConstructorParentHelperPreservesAuthorization(t *testing.T) {
	victimDeleted := false
	ownedDeleted := false
	sql.Register("imported-constructor-helper-parent-safe", witnessDriver{
		victimDeleted: &victimDeleted,
		ownedDeleted:  &ownedDeleted,
	})
	db, err := sql.Open("imported-constructor-helper-parent-safe", "")
	if err != nil {
		t.Fatal(err)
	}
	defer db.Close()
	...
	if response.Code != 204 || victimDeleted {
		t.Fatalf("imported constructor parent helper crossed ownership: code=%d victimDeleted=%v", response.Code, victimDeleted)
	}

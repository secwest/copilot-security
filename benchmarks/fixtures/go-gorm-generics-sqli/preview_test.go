...
func TestGenericWhereInjectionReturnsInternalRecord(t *testing.T) {
	sql.Register("gorm-generics-injection-witness", witnessDriver{})
	database, err := sql.Open("gorm-generics-injection-witness", "")
	if err != nil {
		t.Fatal(err)
	}
	...
	if !strings.Contains(response.Body.String(), "internal-signing-key") {
		t.Fatalf("generic predicate injection did not expose the internal row: %q", response.Body.String())
	}
}

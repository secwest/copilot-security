...
func (connection *witnessConn) ExecContext(_ context.Context, query string, arguments []driver.NamedValue) (driver.Result, error) {
	if query != "DELETE FROM invoices WHERE id = ? AND account_id = ?" || len(arguments) != 2 {
		return nil, errors.New("unexpected helper value-copy pointer mutation")
	}
...
func TestHelperValueCopyPointerPreservesAuthorization(t *testing.T) {
	victimDeleted := false
	ownedDeleted := false
	sql.Register("helper-value-copy-pointer-safe", witnessDriver{victimDeleted: &victimDeleted, ownedDeleted: &ownedDeleted})
	db, err := sql.Open("helper-value-copy-pointer-safe", "")
	if err != nil {
		t.Fatal(err)
	}
	defer db.Close()
	...
	if response.Code != 204 || victimDeleted {
		t.Fatalf("helper value-copy pointer crossed ownership: code=%d victimDeleted=%v", response.Code, victimDeleted)
	}

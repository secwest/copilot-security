...
func (connection *witnessConn) ExecContext(_ context.Context, query string, arguments []driver.NamedValue) (driver.Result, error) {
	if query != "DELETE FROM invoices WHERE id = ? AND account_id = ?" || len(arguments) != 2 {
		return nil, errors.New("unexpected authorized nested-field-write mutation")
	}
...
func TestPrincipalScopeSurvivesNestedConstructorFieldWrite(t *testing.T) {
	owners := map[string]string{
		"victim-invoice": "victim-account",
		"owned-invoice":  "attacker-account",
	}
	sql.Register("safe-constructor-nested-field-write", witnessDriver{owners: owners})
	db, err := sql.Open("safe-constructor-nested-field-write", "")
	if err != nil {
		t.Fatal(err)
	}
	defer db.Close()
	...
	ownedRequest := httptest.NewRequest("DELETE", "/invoices/owned-invoice", nil)
	ownedRequest.SetPathValue("invoiceID", "owned-invoice")
	ownedRequest = ownedRequest.WithContext(context.WithValue(ownedRequest.Context(), authenticatedAccountIDKey, "attacker-account"))
	ownedResponse := httptest.NewRecorder()
	InvoiceDeleteHandler(db, ownedResponse, ownedRequest)
	if ownedResponse.Code != 204 {
		t.Fatalf("owned delete failed: code=%d", ownedResponse.Code)
	}
	if _, exists := owners["owned-invoice"]; exists {
		t.Fatalf("owned invoice was not deleted: owners=%v", owners)
	}
}

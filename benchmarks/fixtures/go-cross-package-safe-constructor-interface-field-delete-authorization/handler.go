...
import (
	"database/sql"
	primary "example.com/go-cross-package-safe-constructor-interface-field-delete-authorization/internal/primary"
	service "example.com/go-cross-package-safe-constructor-interface-field-delete-authorization/internal/service"
	"net/http"
)
...
func InvoiceDeleteHandler(db *sql.DB, w http.ResponseWriter, r *http.Request) {
	...
	invoices := service.NewService(repository)

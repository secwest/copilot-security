...
func (connection *witnessConn) ExecContext(_ context.Context, query string, arguments []driver.NamedValue) (driver.Result, error) {
	if query != "DELETE FROM invoices WHERE id = ? AND account_id = ?" || len(arguments) != 2 {
		return nil, errors.New("unexpected authorized interface-field mutation")
	}
...
func TestPrincipalScopeSurvivesConstructorInjectedInterface(t *testing.T) {
	owners := map[string]string{"victim-invoice": "victim-account", "attacker-invoice": "attacker-account"}
	sql.Register("safe-constructor-interface-field", witnessDriver{owners: owners})
	db, err := sql.Open("safe-constructor-interface-field", "")
	if err != nil {
		t.Fatal(err)
	}
	defer db.Close()

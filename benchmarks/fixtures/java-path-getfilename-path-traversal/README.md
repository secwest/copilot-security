...
This Spring fixture passes a request parameter through a service into a repository. The repository calls a package-access static helper in a separate compilation unit that converts the value with exact `java.nio.file.Path.of` and returns only `Path.getFileName()` before resolving it beneath a document root.

...
This Spring fixture passes a request parameter through a service into a repository. The repository calls a private same-file helper that converts the value with exact `java.nio.file.Path.of` and returns only `Path.getFileName()` before resolving it beneath a document root.

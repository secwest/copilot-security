...
func (connection *witnessConn) ExecContext(_ context.Context, query string, arguments []driver.NamedValue) (driver.Result, error) {
	if query != "DELETE FROM invoices WHERE id = ? AND account_id = ?" || len(arguments) != 2 {
		return nil, errors.New("unexpected cross-package interface type switch helper mutation")
	}
...
func TestCrossPackageInterfaceTypeSwitchHelperPreservesAuthorization(t *testing.T) {
	victimDeleted := false
	ownedDeleted := false
	sql.Register("cross-package-interface-type-switch-helper-safe", witnessDriver{victimDeleted: &victimDeleted, ownedDeleted: &ownedDeleted})
	db, err := sql.Open("cross-package-interface-type-switch-helper-safe", "")
	if err != nil {
		t.Fatal(err)
	}
	defer db.Close()
	...
	if response.Code != 204 || victimDeleted {
		t.Fatalf("cross-package interface type switch helper crossed ownership: code=%d victimDeleted=%v", response.Code, victimDeleted)
	}

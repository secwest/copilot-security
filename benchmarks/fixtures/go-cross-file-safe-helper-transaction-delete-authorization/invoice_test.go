...
type helperTransactionDriver struct {
	state *invoiceState
}

func (driver_ helperTransactionDriver) Open(string) (driver.Conn, error) {
	return &helperTransactionConn{state: driver_.state}, nil
}

type helperTransactionConn struct {
	state  *invoiceState
	active *helperTransactionTx
}

func (*helperTransactionConn) Prepare(string) (driver.Stmt, error) {
	return nil, errors.New("prepared statements are not used")
}
func (*helperTransactionConn) Close() error { return nil }
func (connection *helperTransactionConn) Begin() (driver.Tx, error) {
	if connection.active != nil {
		return nil, errors.New("transaction already active")
	}
	transaction := &helperTransactionTx{connection: connection}
	connection.active = transaction
	return transaction, nil
}
func (connection *helperTransactionConn) ExecContext(_ context.Context, query string, arguments []driver.NamedValue) (driver.Result, error) {
...
type helperTransactionTx struct {
	connection *helperTransactionConn
	staged     []string
	done       bool
}

func (transaction *helperTransactionTx) Commit() error {
	if transaction.done {
...
func (transaction *helperTransactionTx) Rollback() error {
	if transaction.done {
		return errors.New("transaction already finalized")
	}
...
func TestPrincipalPredicateScopesHelperCommittedDeletion(t *testing.T) {
	state := &invoiceState{owners: map[string]string{
		"victim-invoice":   "victim-account",
		"attacker-invoice": "attacker-account",
	}}
	sql.Register("authorized-helper-transaction-witness", helperTransactionDriver{state: state})
	db, err := sql.Open("authorized-helper-transaction-witness", "")
	if err != nil {
		t.Fatal(err)
	}
	defer db.Close()
	...
	if state.owners["victim-invoice"] != "victim-account" {
		t.Fatal("cross-account helper-committed deletion became durable")
	}
	...
	if _, exists := state.owners["attacker-invoice"]; exists {
		t.Fatal("owned helper-committed deletion did not become durable")
	}
}

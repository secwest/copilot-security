...
func (connection *witnessConn) ExecContext(_ context.Context, query string, arguments []driver.NamedValue) (driver.Result, error) {
	if query != "DELETE FROM invoices WHERE id = ? AND account_id = ?" || len(arguments) != 2 {
		return nil, errors.New("unexpected authorized constructor-field-write mutation")
	}
...
func TestPrincipalScopeSurvivesConstructorFieldWrite(t *testing.T) {
	owners := map[string]string{
		"victim-invoice": "victim-account",
		"owned-invoice":  "attacker-account",
	}
	sql.Register("safe-constructor-field-write", witnessDriver{owners: owners})
	db, err := sql.Open("safe-constructor-field-write", "")
	if err != nil {
		t.Fatal(err)
	}
	defer db.Close()

...
func TestPrincipalPredicateScopesHelperChainCommittedDeletion(t *testing.T) {
	state := &invoiceState{owners: map[string]string{
		"victim-invoice":   "victim-account",
		"attacker-invoice": "attacker-account",
	}}
	sql.Register("authorized-helper-chain-transaction-witness", helperChainTransactionDriver{state: state})
	db, err := sql.Open("authorized-helper-chain-transaction-witness", "")
	if err != nil {
		t.Fatal(err)
	}
	defer db.Close()
	...
	if state.owners["victim-invoice"] != "victim-account" {
		t.Fatal("cross-account helper-chain-committed deletion became durable")
	}
	...
	if _, exists := state.owners["attacker-invoice"]; exists {
		t.Fatal("owned helper-chain-committed deletion did not become durable")
	}
}

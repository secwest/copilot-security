...
func TestPrincipalPredicateScopesCommittedDeletion(t *testing.T) {
	state := &invoiceState{owners: map[string]string{
		"victim-invoice":   "victim-account",
		"attacker-invoice": "attacker-account",
	}}
	sql.Register("authorized-transaction-delete-witness", transactionDriver{state: state})
	db, err := sql.Open("authorized-transaction-delete-witness", "")
	if err != nil {
		t.Fatal(err)
	}
	defer db.Close()
	...
	if state.owners["victim-invoice"] != "victim-account" {
		t.Fatal("cross-account deletion was committed")
	}
	...
	if _, exists := state.owners["attacker-invoice"]; exists {
		t.Fatal("owned invoice deletion was not committed")
	}
}

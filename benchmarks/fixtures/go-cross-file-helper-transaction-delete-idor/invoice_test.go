...
func TestAttackerCommitsVictimDeletionThroughHelper(t *testing.T) {
	state := &invoiceState{owners: map[string]string{
		"victim-invoice": "victim-account",
	}}
	sql.Register("helper-transaction-delete-idor-witness", helperTransactionDriver{state: state})
	db, err := sql.Open("helper-transaction-delete-idor-witness", "")
	if err != nil {
		t.Fatal(err)
	}
	defer db.Close()
	...
	if _, exists := state.owners["victim-invoice"]; exists {
		t.Fatal("helper-committed victim deletion did not become durable")
	}
}

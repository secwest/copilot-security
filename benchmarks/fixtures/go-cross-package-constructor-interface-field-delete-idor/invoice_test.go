...
func (connection *witnessConn) ExecContext(_ context.Context, query string, arguments []driver.NamedValue) (driver.Result, error) {
	if query != "DELETE FROM invoices WHERE id = ?" || len(arguments) != 1 {
		return nil, errors.New("unexpected interface-field mutation")
	}
	*connection.deleted = true
	return driver.RowsAffected(1), nil
}

func TestAttackerDeletesVictimThroughConstructorInjectedInterface(t *testing.T) {
	deleted := false
	sql.Register("constructor-interface-field-idor", witnessDriver{deleted: &deleted})
	db, err := sql.Open("constructor-interface-field-idor", "")
	if err != nil {
		t.Fatal(err)
	}
	defer db.Close()
	...
	if response.Code != 204 || !deleted {
		t.Fatalf("constructor-injected interface path did not delete: code=%d deleted=%v", response.Code, deleted)
	}
}

...
func TestAttackerDeletesVictimThroughHelperBranch(t *testing.T) {
	deleted := false
	sql.Register("helper-branch-idor", witnessDriver{deleted: &deleted})
	db, err := sql.Open("helper-branch-idor", "")
	if err != nil {
		t.Fatal(err)
	}
	defer db.Close()
	...
	if response.Code != 204 || !deleted {
		t.Fatalf("helper branch did not delete: code=%d deleted=%v", response.Code, deleted)
	}
}

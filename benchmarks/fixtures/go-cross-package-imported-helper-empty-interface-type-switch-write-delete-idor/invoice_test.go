...
func TestAttackerDeletesVictimThroughEmptyInterfaceTypeSwitchHelper(t *testing.T) {
	deleted := false
	sql.Register("empty-interface-type-switch-helper-idor", witnessDriver{deleted: &deleted})
	db, err := sql.Open("empty-interface-type-switch-helper-idor", "")
	if err != nil {
		t.Fatal(err)
	}
	defer db.Close()
	...
	if response.Code != 204 || !deleted {
		t.Fatalf("empty-interface type switch helper did not delete: code=%d deleted=%v", response.Code, deleted)
	}
}

...
import (
	"database/sql"
	service "example.com/go-cross-package-safe-wrapper-chain-delete-authorization/internal/invoicesvc"
	"net/http"
)
...
func InvoiceDeleteHandler(db *sql.DB, w http.ResponseWriter, r *http.Request) {
	...
	if err := service.DeleteInvoice(r.Context(), db, invoiceID, accountID); err != nil {

...
ASSET_URLS = {
    "logo": "https://assets.example.internal/logo.svg",
    "theme": "https://assets.example.internal/theme.css",
}
MAX_PREVIEW_BYTES = 64 * 1024


def fetch_preview(asset):
    if asset not in ASSET_URLS:
        raise KeyError("unknown asset")
    with requests.get(
        ASSET_URLS[asset], allow_redirects=False, stream=True, timeout=(2, 2)
    ) as response:

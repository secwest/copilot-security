...
func TestAttackerDeletesVictimThroughImportedHelperWrite(t *testing.T) {
	deleted := false
	sql.Register("imported-helper-write-idor", witnessDriver{deleted: &deleted})
	db, err := sql.Open("imported-helper-write-idor", "")
	if err != nil {
		t.Fatal(err)
	}
	defer db.Close()
	...
	if response.Code != 204 || !deleted {
		t.Fatalf("imported helper write did not delete: code=%d deleted=%v", response.Code, deleted)
	}
}

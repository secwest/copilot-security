...
func (connection *witnessConn) ExecContext(_ context.Context, query string, arguments []driver.NamedValue) (driver.Result, error) {
	if query != "DELETE FROM invoices WHERE id = ? AND account_id = ?" || len(arguments) != 2 {
		return nil, errors.New("unexpected authorized branch-joined mutation")
	}
...
func TestPrincipalScopeSurvivesJoinedConstructorBranches(t *testing.T) {
	owners := map[string]string{
		"victim-invoice": "victim-account",
		"owned-invoice":  "attacker-account",
	}
	sql.Register("safe-constructor-branch-write", witnessDriver{owners: owners})
	db, err := sql.Open("safe-constructor-branch-write", "")
	if err != nil {
		t.Fatal(err)
	}
	defer db.Close()

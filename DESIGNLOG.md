...
**Follow-up.** The successful session still recorded eight failed creates against absent artifact parents and three failed views of not-yet-created JSON files before recovering to 38 successful sandboxed tool calls. Host-side preparation should create the authorized scan-contract directory skeleton before the model turn, reducing wasted operations without granting broader filesystem access or weakening missing-artifact validation.

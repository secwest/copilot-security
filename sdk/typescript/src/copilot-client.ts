  ...
  return [
    ...
    ...(residualRiskInventory === ""
      ...
      : [
          ...
          "For python-web-pyyaml-unsafe-load rows, reopen the exact request-body source, every recorded relative wrapper, the Python yaml import, the stream argument, and the explicit unsafe API or Loader. The host accepts only a non-reassigned import yaml receiver or from yaml import binding, rejects a repository-local yaml.py or yaml package shadow, and emits only unsafe_load or load with an explicit Loader, UnsafeLoader, CLoader, or CUnsafeLoader. It deliberately rejects safe_load, SafeLoader, CSafeLoader, full_load, FullLoader, load without an explicit proven loader, fixed YAML, another argument, reassigned imports or members, and text-only lookalikes. Verify that the deployed yaml module is PyYAML, the recorded input retains attacker-controlled YAML tags and bytes, and deserialization is reached. PyYAML Loader and UnsafeLoader variants can construct Python-specific objects during load; parsing success, later schema validation, authentication, try/except, or discarding the returned value does not undo constructor side effects. SafeLoader is strong counterevidence for arbitrary-object construction, but separately inspect parser-complexity and application-level data validation. Validate with a bounded non-destructive payload against the exact environment, identify the available constructor or gadget, process privileges and containment, error handling, and one concrete execution, filesystem, network, credential, integrity, or availability effect. Report CWE-502 and only the demonstrated impact; do not claim code execution from the host row, an import name, or unsafe-loader selection alone.",

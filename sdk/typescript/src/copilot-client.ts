...
export function scanQualityGatePrompt(
  ...
  return [
    ...
    "For child_process.fork role-specific rows, reopen the exact live official node:child_process or child_process fork binding and preserve all four boundaries for the direct roles: modulePath selects executable child-module code, options.execPath selects the executable that creates the child, options.execArgv supplies Node runtime options before that module, and ordinary args are module data. Also inspect the two execution-context boundaries that can redirect a fixed-looking call: options.cwd controls resolution of a non-absolute modulePath, and options.env.NODE_OPTIONS supplies Node runtime options including preload flags. For mcp-tool-fork-module-path, require tool input in argument zero, repeat the recorded fork:modulePath symbol in both validation and attackPath, report node-mcp-tool-untrusted-module-load with CWE-829, and use a fixed or explicit allowlisted child module as the repair; do not claim CWE-94, arbitrary code placement, or code execution beyond an existing selected module unless a separate chain proves it. For mcp-tool-fork-relative-cwd, require a fixed non-absolute modulePath plus tool input in exact options.cwd, repeat the recorded options.cwd->relative-modulePath symbol in both fields, report node-mcp-tool-untrusted-module-load with CWE-426 and CWE-829, and compare an absolute path/file URL or fixed allowlisted working directory. For mcp-tool-fork-exec-path, require tool input in the exact object-literal options.execPath property, repeat the recorded fork:options.execPath symbol in both fields, report CWE-78 without CWE-88, and compare the current process.execPath while keeping the same value only in ordinary child arguments. For mcp-tool-fork-node-options, require tool input in exact options.env.NODE_OPTIONS and a default or exactly proven process.execPath runtime, repeat the recorded options.env.NODE_OPTIONS symbol in both fields, report node-mcp-tool-argument-injection with CWE-88/CWE-94, and compare fixed NODE_OPTIONS or the same value in a non-special environment variable. A preceding process.env spread inside the exact env literal is compatible because the final explicit NODE_OPTIONS wins; a following spread is ambiguous and must be rejected. Reject replaced fork bindings or namespace members, dynamic or aliased option objects, computed or duplicate relevant properties, unsupported overloads, unknown custom execPath values, and input confined to a different fork role. Validate cwd resolution only with checked-in fixture-local benign children, and validate NODE_OPTIONS only with one checked-in inert preload that sets a private child marker. For mcp-tool-node-options on spawn, spawnSync, execFile, or execFileSync, require a live official child-process binding, an exactly proven process.execPath executable, exact non-shell options, and tool input in options.env.NODE_OPTIONS; repeat the recorded boundary in both fields and compare fixed NODE_OPTIONS or the same bytes in an ordinary environment variable. For mcp-tool-executable-search-path, require a fixed bare executable plus tool input in exact options.env.PATH, repeat the PATH-to-command boundary in both fields, report node-mcp-tool-untrusted-executable-search with CWE-426, and compare an absolute executable, fixed allowlisted PATH, or ordinary environment data. Do not claim that PATH bytes are command source or that attacker code exists without a separately proved attacker-writable directory and shadow executable. For both environment models, reject shells, aliased or ambiguous options, unsupported overloads, computed or duplicate properties, and following spreads. Use only an inert checked-in Node preload or a bounded missing-command lookup in witnesses. Never create or select an external attacker module, arbitrary executable, loader, inspector, shell, filesystem writer, network client, credential, persistence mechanism, or attacker code.",
    ...
    "For spring-r2dbc-sql rows, preserve the exact org.springframework.r2dbc.core.DatabaseClient receiver, request source, every Java wrapper argument and parameter, the value occupying DatabaseClient.sql's SQL-grammar argument, and the recorded fetch, then, map, flatMap, mapValue, or mapProperties execution-stage transition. DatabaseClient.sql accepts direct SQL text or a Supplier; trace the Supplier result rather than treating the lambda object as safe. A fixed server-owned SQL statement with the same request value passed only through bind or bindNull is strong SQL-injection counterevidence because binding keeps data out of query grammar. Reject local or competing DatabaseClient types, reassigned receivers, malformed or multi-argument lookalikes, request data used only as a bound value, and an inert sql specification that never enters a documented execution stage. The recorded transition establishes query execution setup, not by itself subscription or completion: verify that the returned Publisher is consumed or returned into an active reactive chain. Before reporting CWE-89, establish the deployed R2DBC driver and database dialect, quoting and comment behavior, multi-statement policy, the exact predicate or statement change, database role, transaction and error behavior, tenant and object authorization, returned data, and a concrete unauthorized read, write, or availability effect. Parameterization fixes value positions; dynamic identifiers or clauses require fixed server-owned selection or a context-correct allowlist.",
    "For java-r2dbc-spi-sql rows, preserve the exact io.r2dbc.spi Connection, Statement, or Batch receiver type, the request source and every Java wrapper edge, the precise grammar position, and the recorded execution or returned-Publisher transition. Connection.createStatement and Batch.add accept SQL text; Statement.bind or bindNull keeps the same request value out of SQL grammar and is strong counterevidence for a value position. Statement.add has no SQL argument and must not be confused with Batch.add. Connection createSavepoint, releaseSavepoint, and rollbackTransactionToSavepoint pass an identifier to the driver, while Statement.returnGeneratedValues passes generated-column identifiers; require driver-specific proof that hostile syntax in that exact identifier position changes SQL or database behavior rather than assuming every identifier is injectable. An inert Statement or Batch, an unreturned savepoint Publisher, or execute without Publisher subscription and proof that its Result objects are fully consumed does not prove database execution. Reject local or competing SPI lookalikes, ambiguous imports, wrong receiver types or arity, reassigned receivers or assigned statements, fixed grammar, and request data used only as a bound value. Before reporting CWE-89, verify the deployed driver and dialect, bind-marker and identifier quoting rules, multi-statement policy, transaction and auto-commit state, Result consumption, database role, authorization boundary, and a concrete unauthorized read, write, schema, transaction, or availability effect. A statement timeout limits duration but does not make attacker-controlled SQL or identifiers safe; an allowlist is counterevidence only when the validated server-owned value reaches the sink.",
    "For python-asyncpg-sql rows, preserve the exact live official asyncpg Connection or Pool receiver, request source, copied query variables and every Python wrapper edge, the grammar-bearing first query or command argument, and actual await or async cursor consumption. Review copy_from_query, cursor, execute, executemany, fetch, fetchmany, fetchrow, fetchval, and prepare using their exact signatures; copy_from_query is spelled with underscores, and fetchmany is a distinct current API. Later *args or args values are PostgreSQL protocol-bound values, so a fixed server-owned statement with $1 placeholders and the same request value only in those later arguments is strong SQL-injection counterevidence. A coroutine created without await and a cursor factory never awaited or iterated are not execution. Reject local asyncpg lookalikes, shadowed or reassigned imports, replaced factory members, unproved receivers, malformed call shapes, request data only in bound values, and fixed query grammar. Before reporting CWE-89, verify the exact asyncpg, Python, PostgreSQL server and proxy versions; simple versus extended protocol behavior; whether the method prepares or permits multiple commands; quoting, comments, type codecs and transaction state; database role and row or tenant authorization; returned or changed data; and a concrete unauthorized read, write, schema, transaction, or availability effect. The recording benchmark witness proves only query-argument separation and never contacts PostgreSQL; do not present it as a database exploit.",
    ...
    ...(residualRiskInventory === ""
      ...
      : [
          ...
          "For node-mcp-tool-code-injection construction-lifecycle rows, require the recorded live global Function, exact vm.compileFunction, new vm.Script, new vm.SourceTextModule, or exact node:worker_threads/worker_threads new Worker construction and the applicable execution lifecycle. A compiled Function must be invoked directly or through its live call/apply method; a Script must reach runInContext, runInNewContext, or runInThisContext; a SourceTextModule must complete an awaited legacy link before evaluate or modern linkRequests then instantiate then evaluate. A Worker row must prove that tool input reaches constructor argument zero and an exact object-literal options argument contains one literal eval: true property: Node then interprets the first argument as JavaScript and executes it once the worker is online, so no later method call is required. Reject bare Function/vm construction, unawaited legacy linking, incomplete module lifecycles, Worker calls without new, omitted/false/dynamic/duplicate eval options, spread or computed options, tool input confined to workerData, lookalikes, and replaced imports, constructor members, compiled values, prototype methods, or instance methods. Name construction and execution or worker startup separately in validation and attackPath. Node documents that node:vm is not a security mechanism, while vm.Script construction compiles without running, vm.Module use has distinct creation, linking, and evaluation stages, and Worker eval mode executes the code string once online. Validate only with fixed side-effect-free arithmetic; for Worker use a topology-matched control with fixed server-owned worker source and pass parsed values solely through structured-cloned workerData.",
          ...
          "For python-web-command rows with a python-list-append-element, python-list-extend-element, python-list-insert-element, or python-list-iadd-element propagator, reopen the exact initially empty list, the recorded mutation, the constant indexed element selected by the shell sink, and every relative-import wrapper edge. Prove that the wrapper parameter occurs in that selected element after bounded alias resolution and that no parameter reassignment, collection reassignment, element overwrite, clear, sort, reverse, pop, remove, or second mutation intervenes. An append contributes one element; extend and += contribute only their exact literal elements; insert into the proven empty list contributes its inserted value at index zero regardless of the requested insertion position. Negative indexes select from the end of the bounded literal. Reject nonempty or unknown initial collections, dynamic indexes, unsupported comprehensions or iterable expansion, a hostile value stored only in another element, mutation in comments or strings, and request data used only as literal argv with shell=False. Validation and attackPath must each name the framework request source, relative wrapper, exact list operation, selected index, shell=True or os.system grammar boundary, CWE-78, and the absence of an intervening overwrite. For python-dict-literal-element, python-dict-item-assignment-element, python-dict-update-element, python-dict-ior-element, or python-dict-setdefault-element, reopen the exact dictionary literal or empty dict(), every recorded constant-key mutation, the bracket/get/pop selection, and the relative wrapper. Reconstruct deterministic last-write-wins dictionary state: direct assignment, update, and |= overwrite a key, while setdefault preserves an existing key. Require the wrapper parameter in the value selected by the exact constant key after bounded value and sink-alias resolution. Reject dynamic or escaped keys, unpacking, comprehensions, keyword-form dict construction, nonliteral update operands, unknown method calls, dictionary escape through an alias or helper, parameter reassignment before storage, comments or strings, and taint confined to an unselected or overwritten value. Validation and attackPath must each name the exact dictionary operation, constant key, selected dictionary value, deterministic state, absence of an ambiguous mutation or intervening overwrite, shell boundary, relative wrapper, request source, and CWE-78. The paired witness proves only POSIX shell expansion of a harmless temporary marker; confirm deployed platform, executable, privileges, reachability, authentication, containment, and a concrete effect before escalating impact.",
          ...
          "For node-mcp-tool-command-injection, node-mcp-tool-argument-injection, node-mcp-tool-code-injection, node-mcp-tool-path-traversal, and node-mcp-tool-ssrf rows, reopen the exact official MCP TypeScript SDK import, McpServer construction, live server alias, schema-bearing tool registration, callback input binding, every recorded local assignment or same-file helper call, and the exact process, code-evaluation, filesystem, or network sink. When a row records an mcp-tool-helper-call, both validation and attackPath must repeat the exact helper symbol or explicitly call it the same-file helper/helper call; evidence in only one field is insufficient. Stable v2 registerTool callbacks receive tool input only when inputSchema is present; a schema-less callback receives context, while legacy v1 variadic server.tool uses its explicit schema before the callback. In both validation and attackPath, explicitly name the MCP client, model, tool caller, or tool invocation that can supply the callback input, and name inputSchema or schema validation plus the exact property the schema proves and the security property it does not prove. Schema validation proves shape, not that an arbitrary string is safe shell grammar, an executable name or interpreter option, JavaScript source, a filesystem path, or a network destination. For command rows, distinguish child_process exec/execSync shell strings, shell-enabled spawn/execFile, fixed shell interpreter arguments, and attacker-selected executables from fixed executable plus separate argv data. Fixed executable plus argv is strong shell-injection counterevidence, but when the executable is process.execPath or one recorded stable process.execPath alias, explicitly reopen that exact runtime assignment and verify that an exact -- end-of-options element precedes every untrusted argv element; without it, a dash-prefixed value remains in the Node option region as an interpreter option and is argument injection. Reject a local process shadow, process.execPath replacement, reassigned or multiply declared runtime alias, alias chains, nested ambiguous aliases, and unsupported runtime expressions rather than inferring Node identity. In both validation and attackPath, repeat the recorded runtime-alias symbol when present and use option region, end-of-options, or argument injection wording rather than relying on the bare -- token to explain this boundary. For code rows, prove that tool input reaches the code argument of the live unshadowed global eval or an exact vm/node:vm runInContext, runInNewContext, or runInThisContext binding. Node documents that node:vm is not a security mechanism. A Function constructor, vm.compileFunction, new vm.Script, or new vm.SourceTextModule that is never invoked or evaluated is inert construction and must not be reported as executed code; require an explicit execution step before escalating such a separate lead. A string schema, syntax check, timeout, context object, or result-type check does not turn arbitrary source into data. Prefer a fixed grammar or operator allowlist implemented without dynamic JavaScript evaluation. Validate only with fixed side-effect-free arithmetic such as 6 * 7 and a topology-matched non-evaluating parser control; never use shell, filesystem, network, credential, persistence, privilege, or destructive effects. For path rows, prove that tool input reaches a path-bearing Node fs or fs/promises argument rather than file contents, encoding, flags, or callback data. path.join or path.resolve alone normalizes syntax but does not prove confinement; require an exact allowlist or a boundary-safe canonical containment design that accounts for absolute paths, separators, symlinks, and check/use races. A fixed operator path with tool input confined to write contents is strong counterevidence. For SSRF rows, prove that tool input selects the URL, host, hostname, protocol, or Axios baseURL rather than merely a request body or same-host path; a literal operator-pinned destination with input confined to body data is strong counterevidence. Reject local SDK/process/vm/fs/network lookalikes, comments and strings, tests and generated files, schema-less v2 context callbacks, inert registrations, overwritten taint, unrelated helper parameters, fixed code, paths, or destinations, and unsupported callback forms. Confirm the server is launched, transport and authentication policy, which client or model can invoke the tool, schema and authorization controls, process privilege, evaluation context and exposed capabilities, filesystem permissions and confinement, network egress, redirect and DNS behavior, destination parsing, timeouts and response-size limits, and the least concrete effect. A host row proves a candidate tool-input edge, not anonymous Internet reachability, successful command execution, arbitrary host-file access, metadata access, credential disclosure, persistence, or full host compromise. Validate command or argument behavior only with bounded inert fixed-string argv; validate code evaluation only with fixed side-effect-free arithmetic; validate filesystem behavior only inside a newly created disposable temporary tree containing synthetic marker data; validate network behavior only with a disposable loopback endpoint and fake data when stronger proof is necessary. Report CWE-78/CWE-88 for shell-command rows, CWE-88/CWE-94 for Node interpreter-option rows, CWE-94/CWE-95 for code-evaluation rows, CWE-22/CWE-73 for path rows, and CWE-918 for SSRF rows.",
          "For mcp-tool-node-process-binding propagators, reopen the exact official node:process default, namespace, named execPath, CommonJS, or TypeScript import-equals binding and the recorded runtime expression. In both validation and attackPath, repeat the exact binding symbol and independently name the official process module; a runtime-alias statement alone is insufficient. Reject the unprefixed process package, local or unsupported module lookalikes, duplicate or shadowed bindings, binding or execPath mutation, destructuring reassignment, computed members, alias chains, and ambiguous nested aliases. Preserve the exact -- end-of-options requirement after establishing runtime identity.",
          "For mcp-tool-fork-exec-argv propagators, reopen the exact official child_process fork binding, fixed module path, selected fork overload, object-literal options argument, and recorded options.execArgv array element. child_process.fork creates a new Node process and execArgv is interpreted by the Node executable before the fixed module; ordinary args follow the module as data. In both validation and attackPath, repeat the recorded fork:options.execArgv symbol and distinguish the matched control: fixed execArgv with the same tool value only in ordinary module arguments. Reject wrong modules, replaced fork bindings or namespace members, dynamic or aliased options objects, spreads, computed or duplicate execArgv properties, non-array execArgv values, input confined to ordinary args, and unsupported overloads. A literal -- inside execArgv is not the matched repair because following entries can change which module Node executes. Validate only with a fixed inert runtime option such as --stack-trace-limit=77 and the topology-matched ordinary-argument control; never use --require, --import, loaders, inspectors, external modules, filesystem writes, network access, credentials, persistence, or attacker code.",
          ...
          "For rust-web-command-injection rows whose sink symbol names tokio::process::Command, require that exact official Tokio identity rather than accepting a local tokio module, lookalike crate alias, or std-only explanation. Preserve the same shell-versus-ordinary-argv distinction and actual output, status, or spawn call required for std; Tokio does not make a fixed executable plus one remote literal argument into shell grammar. Current Tokio status() and output() call spawn() before returning their Future, so missing await is not execution prevention: confirm the deployed Tokio version, child lifetime and kill_on_drop policy, standard-stream behavior, whether the future is polled, and the least concrete child-process effect. CodeQL's August 2026 model treats tokio new, arg, and args as command-injection sinks but its own help recommends separate arguments; do not inherit a generic argument alert when the exact consumer preserves literal argv. Validate only with the same bounded inert marker and topology-matched Tokio control used for std command rows.",
          ...
          "For python-web-command rows with a python-object-constructor-field, python-object-attribute-assignment, or python-object-setattr-assignment propagator, reopen the exact fresh standard-library types.SimpleNamespace construction, official import binding, recorded constant-field write, same receiver and field read, shell sink, and every relative-import wrapper edge. Reconstruct receiver-sensitive last-write-wins field state across at most 64 fields and 64 transitions. Constructor keywords establish initial fields; direct attribute assignment and constant-name setattr overwrite one field. Require the wrapper parameter in the selected field after bounded value and sink-alias resolution. Reject absent, replaced, duplicated, or shadowed SimpleNamespace bindings; arbitrary classes; positional constructor arguments; dynamic or __dict__ fields; unknown methods; augmented, annotated, deleted, or ambiguous writes; object escape through an alias or helper; receiver or parameter reassignment; comments or strings; and taint confined to another receiver or field or removed by a later safe overwrite. Validation and attackPath must each name the framework request source, relative wrapper, fresh SimpleNamespace receiver, exact field operation, constant selected field, same receiver, receiver-sensitive last-write-wins field state, absence of alias escape or intervening overwrite, shell=True or os.system grammar boundary, CWE-78, and the topology-matched field-isolation control. The paired witness proves only POSIX shell expansion of a harmless temporary marker; confirm deployed platform, executable, privileges, reachability, authentication, containment, and a concrete effect before escalating impact.",
    ...
    "For node-mcp-tool-sql-injection rows, reopen the exact MCP registration, callback input property, inputSchema, every recorded assignment or same-file helper, official node:sqlite DatabaseSync construction, and the recorded SQL lifecycle. A direct row requires tool-controlled SQL in argument zero of live DatabaseSync.exec. A prepared row requires tool-controlled SQL in argument zero of live DatabaseSync.prepare plus execution of that exact returned StatementSync through run, get, all, or iterate; preparation alone is not execution. The model requires a stable module-scope built-in database instance and a live receiver chain; reject sqlite3 or local lookalikes, constructor options whose open state is not proven, dynamic or callback-local construction, closed or disposed databases and statements, receiver or method replacement, shadowed names, and unsupported statement flows. Tool input passed only to StatementSync run, get, all, or iterate as a value for fixed server-owned placeholder SQL is strong counterevidence. A string or length schema proves shape, not SQL grammar. In validation and attackPath, name the MCP caller or tool invocation, exact tool property and schema, same-file helper when present, node:sqlite and DatabaseSync identity, DatabaseSync.exec or the DatabaseSync.prepare to StatementSync execution sequence, SQL argument zero, interpolation or another structure-changing flow, prepared-statement parameter binding as the matched control, CWE-89, database privileges, and the least concrete unauthorized read or write effect. Determine whether stacked statements are supported on the exact path; do not infer exec-style multi-statement behavior from prepare. Distinguish the in-memory fixture witness from deployment evidence. Validate only with a disposable in-memory database and fixed inert rows; never target a file-backed, remote, credential-bearing, or production database.",
    "For node-mcp-tool-regex-injection rows, reopen the exact MCP registration, callback input property, inputSchema, every recorded assignment or same-file helper, live global RegExp construction, later .test or .exec execution, and actual match subject. Argument zero must be the tool-controlled pattern; tool input used only as test data or flags is not this defect, and inert constructor-only code, shadowed or reassigned RegExp, a replaced regex variable, or replaced test/exec method is not execution proof. A string or length schema constrains shape or size, not regular-expression metacharacter grammar or worst-case matching complexity. Reject availability claims when every subject is a small fixed server-owned string and no other confidentiality, integrity, authorization, or concrete availability impact follows. Retain ReDoS only when subject size, repetition, or attacker influence proves meaningful nonlinear matching work on a shared synchronous event loop. Prefer an exact allowlist that selects fixed operator-owned regular-expression literals; a candidate escaping repair is acceptable only when source review proves the complete metacharacter escape is correct and dominates every construction. Both validation and attackPath must name the MCP caller or tool invocation, pattern property and schema limit, same-file helper when present, RegExp construction, actual test/exec execution, match-subject provenance and work bound, injected pattern grammar, CWE-400/CWE-730, and the concrete effect on other work sharing Node's event loop without claiming public reachability or host compromise. Dynamic validation may use only short fixed metacharacter examples such as ^error:, warning|error, or invalid [ plus the topology-matched fixed-pattern control; never execute a catastrophic-backtracking or load-generating expression.",

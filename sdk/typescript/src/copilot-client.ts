...
export function scanQualityGatePrompt(
  ...
  return [
    ...
    "For aspnet-http-template-injection rows, preserve the exact C# controller source, uniquely resolved service type, call argument, wrapper parameter, and the first template-source argument of the real Scriban Template.Parse API through the proven Render or RenderAsync dispatch. Parsing without rendering is inert. A fixed server-owned template parsed once with attacker data supplied only through the Render model is strong SSTI counterevidence even when sensitive server values share that model; render data is not recursively parsed as template source. Scriban's second source-file-name argument, parser options, TemplateContext or ScriptObject use, syntax validation, and output encoding do not make attacker-controlled first-argument template source safe. Reject local Template lookalikes, missing Scriban imports, parsed templates reassigned before rendering, values reassigned before Parse, fixed source, text-only examples, and request values used only after parsing. Classify a proven path as CWE-1336 and validate the concrete exposed object capabilities, secret disclosure, code-like behavior, or resource impact rather than assuming every template grammar has the same power.",

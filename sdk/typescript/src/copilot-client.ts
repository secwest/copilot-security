...
export function scanQualityGatePrompt(
  ...
  return [
    ...
    "For node-http-prototype-merge rows, preserve either an exact official core lodash or lodash/merge binding or a direct default/CommonJS binding from the standalone lodash.merge package, the matching nearest package.json runtime dependency, exact package-specific vulnerable-version evidence, every wrapper boundary, and remote flow into one or more merge source operands; request data used only as the target is outside this contract. Core lodash must resolve below 4.17.11; standalone lodash.merge must resolve below 4.6.2. Version evidence is either an exact numeric runtime declaration or a registry semver declaration whose adjacent npm package-lock.json or higher-precedence npm-shrinkwrap.json uses lockfileVersion 2 or 3, repeats that declaration exactly in packages[''], and records an exact vulnerable version at the corresponding packages['node_modules/<package>'] entry. Never let a lodash declaration authorize a lodash.merge import or the reverse. Prove recursive traversal of __proto__ or constructor.prototype into Object.prototype or another security-relevant prototype and a concrete inherited-property, authorization, code/template execution, denial-of-service, or other effect before classifying CWE-1321. Core Lodash 4.17.11 and standalone lodash.merge 4.6.2 or later are patched for their exact merge primitives; a patched resolution, stale or v1 lockfile, tag, alias, lockfile-free range or workspace declaration, devDependency, missing, malformed, oversized, or symlinked metadata, lookalike package, unsupported namespace or named standalone import, reassigned receiver or merge binding, shallow Object.assign, or object spread does not satisfy this exact model. Independently inspect lockfile resolution, parser behavior, schema rejection of dangerous keys, null-prototype destinations, own-property checks, and whether every source operand is governed by the claimed control. Other merge packages, jQuery/extend families, application-defined recursive utilities, and non-npm dependency graphs require their own version and call-semantics proof.",

  ...
  return [
    ...
    ...(residualRiskInventory === ""
      ...
      : [
          ...
          "For node-mcp-tool-command-injection and node-mcp-tool-ssrf rows, reopen the exact official MCP TypeScript SDK import, McpServer construction, live server alias, schema-bearing tool registration, callback input binding, every recorded local assignment or same-file helper call, and the exact process or network sink. Stable v2 registerTool callbacks receive tool input only when inputSchema is present; a schema-less callback receives context, while legacy v1 variadic server.tool uses its explicit schema before the callback. Schema validation proves shape, not that an arbitrary string is safe shell grammar, an executable name, or a network destination. For command rows, distinguish child_process exec/execSync shell strings, shell-enabled spawn/execFile, fixed shell interpreter arguments, and attacker-selected executables from fixed executable plus separate argv data; the latter is strong counterevidence unless a modeled interpreter or shell consumes it. For SSRF rows, prove that tool input selects the URL, host, hostname, protocol, or Axios baseURL rather than merely a request body or same-host path; a literal operator-pinned destination with input confined to body data is strong counterevidence. Reject local SDK/process/network lookalikes, comments and strings, tests and generated files, schema-less v2 context callbacks, inert registrations, overwritten taint, unrelated helper parameters, fixed destinations, and unsupported callback forms. Confirm the server is launched, transport and authentication policy, which client or model can invoke the tool, schema and authorization controls, process privilege or network egress, redirect and DNS behavior, destination parsing, timeouts and response-size limits, and the least concrete effect. A host row proves a candidate tool-input edge, not anonymous Internet reachability, successful command execution, metadata access, credential disclosure, persistence, or full host compromise. Validate command behavior only with bounded inert fixed-string argv; validate network behavior only with a disposable loopback endpoint and fake data when stronger proof is necessary. Report CWE-78/CWE-88 for command rows and CWE-918 for SSRF rows.",

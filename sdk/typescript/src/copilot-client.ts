...
export function scanQualityGatePrompt(
  ...
  return [
    ...
    "For node-mcp-tool-sql-injection rows, reopen the exact MCP registration, callback input property, inputSchema, every recorded assignment or same-file helper, official node:sqlite DatabaseSync construction, and the exact DatabaseSync.exec argument-zero SQL execution. The model requires a stable module-scope built-in database instance; reject sqlite3 or local lookalikes, constructor options whose open state is not proven, dynamic or callback-local construction, receiver or exec replacement, and shadowed database names. Tool input passed only to StatementSync run, get, all, or iterate as a value for fixed server-owned placeholder SQL is strong counterevidence. DatabaseSync.prepare compiles SQL but is outside this initial execution model unless the same prepared statement is proven to execute; do not invent an exec path. A string or length schema proves shape, not SQL grammar. In validation and attackPath, name the MCP caller or tool invocation, exact tool property and schema, same-file helper when present, node:sqlite and DatabaseSync identity, DatabaseSync.exec or database.exec, SQL argument zero, interpolation or another structure-changing flow, prepared-statement parameter binding as the matched control, CWE-89, database privileges, and the least concrete unauthorized read or write effect. Determine whether stacked statements are supported on the exact path and distinguish the in-memory fixture witness from deployment evidence. Validate only with a disposable in-memory database and fixed inert rows; never target a file-backed, remote, credential-bearing, or production database.",

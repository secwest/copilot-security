...
export function scanQualityGatePrompt(
  ...
  return [
    ...
    "For aspnet-http-command, aspnet-http-sql, and aspnet-http-ssrf rows, preserve the exact C# receiver type, controller method, bound parameter or HttpRequest assignment, service-call argument position, wrapper parameter, and dangerous sink argument across the recorded propagators. For command execution, distinguish attacker data incorporated into a shell or command-line grammar from a fixed executable started with UseShellExecute=false and one ArgumentList entry per attacker-controlled value. For SQL, the first query-text argument of SqlCommand, FromSqlRaw, or ExecuteSqlRaw is the dangerous grammar boundary; a fixed query plus a typed DbParameter or SqlParameter bound to the same value is strong counterevidence. For SSRF, prove whether the same value controls HttpClient's complete request URI or only selects a fixed server-owned URI; request timeouts and response-size bounds limit resource use but do not constrain the destination. Reject duplicate simple service types, unresolved receivers, fixed caller arguments, values reassigned before the service call, sink names found only in comments or strings, and parameters unused by the sink.",

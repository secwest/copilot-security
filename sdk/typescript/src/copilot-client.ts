...
export function scanQualityGatePrompt(
  ...
  return [
    ...
    "For node-mcp-tool-regex-injection rows, reopen the exact MCP registration, callback input property, inputSchema, every recorded assignment or same-file helper, live global RegExp construction, and the later .test or .exec execution. Argument zero must be the tool-controlled pattern; tool input used only as test data or flags is not this defect, and inert constructor-only code, shadowed or reassigned RegExp, a replaced regex variable, or replaced test/exec method is not execution proof. A string or length schema constrains shape or size, not regular-expression metacharacter grammar or worst-case matching complexity. Prefer an exact allowlist that selects fixed operator-owned regular-expression literals; a candidate escaping repair is acceptable only when source review proves the complete metacharacter escape is correct and dominates every construction. Both validation and attackPath must name the MCP caller or tool invocation, property and schema limit, same-file helper when present, RegExp construction, actual test/exec execution, injected pattern grammar, CWE-400/CWE-730, and the concrete synchronous Node event-loop denial-of-service effect without claiming public reachability or host compromise. Dynamic validation may use only short fixed metacharacter examples such as ^error:, warning|error, or invalid [ plus the topology-matched fixed-pattern control; never execute a catastrophic-backtracking or load-generating expression.",

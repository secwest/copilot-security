...
export function scanQualityGatePrompt(
  ...
  return [
    ...
    "For spring-http-ssrf rows, preserve the exact Spring-bound parameter or servlet assignment, both uniquely resolved Java service receiver types, call arguments and parameters, and the request object or complete URI reaching JDK HttpClient.send/sendAsync or a typed RestTemplate operation. URI.create, new URI, HttpRequest.newBuilder, URL encoding, and parsing are not destination authorization. HttpClient.Redirect.NEVER constrains only responses after the initial request and does not make an attacker-controlled initial URI safe. Strong counterevidence is exact request-key selection from fixed server-owned complete destinations plus redirect rejection; a parsed host allowlist additionally requires every DNS A/AAAA answer, connection-time resolution and reuse, proxies, the actual socket address, and Host/TLS identity to be bound or revalidated. Reject URL substring checks, scheme-only checks, locally shadowed HttpClient or RestTemplate types, duplicate service types, fixed caller arguments, reassigned values, and parameters that do not reach the actual request destination.",

  ...
  return [
    ...
    ...(residualRiskInventory === ""
      ...
      : [
          ...
          "For kotlin-ktor-command-injection rows, reopen the exact Ktor route handler and recorded call.request query, header, path, query-string, or receive source, every named Kotlin assignment, concatenation, interpolation, and validation candidate, the exact java.lang.ProcessBuilder command list, and start() on the same non-reassigned builder. In validation, explicitly restate the concrete call expression and key, each named propagation step, the exact ProcessBuilder arguments, and start dispatch. In the attack path, carry that same request value through the named commandLine or equivalent expression into executable selection or the explicit shell, interpreter, PowerShell, or Windows batch grammar position and then to the observed response or effect. ProcessBuilder's first list element selects the executable; attacker control there is executable selection. Ordinary later elements are distinct operating-system arguments, so a fixed ordinary executable with request data only in a separate argument is strong shell-injection counterevidence. The modeled exceptions are POSIX shell -c, cmd /c or /k, PowerShell or pwsh -Command, interpreter code flags such as python -c or node -e, and Windows .bat/.cmd consumers. Require exact Ktor routing imports, an exact explicit java.lang.ProcessBuilder import or fully qualified identity, source and start in the same route lambda, and actual start(); reject tests, examples, fixtures, local lookalikes, construction without execution, reassigned builders, fixed commands, numeric normalization, and request data used only as ordinary argv. Treat regexes, membership tests, escaping helpers, and nearby validation names as candidate controls until source-backed dominance and grammar-specific correctness are proved. Confirm the route is installed, deployed, and attacker reachable; execution reaches start; the target OS and shell/interpreter version; executable and PATH resolution; working directory, environment, standard streams, timeout and resource behavior; process privilege and containment; and the least concrete impact. The host row proves a candidate command-language or executable-selection edge, not successful exploitation, public reachability, persistence, privilege escalation, credential access, arbitrary file access, or complete host compromise. Validate only with a bounded harmless environment-marker witness that preserves the command shape and performs no file, network, credential, persistence, or privilege operation. Report CWE-78 and CWE-88 and calibrate severity from demonstrated reachability, influence, execution context, and impact.",

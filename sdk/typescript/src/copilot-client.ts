...
export function scanQualityGatePrompt(
  ...
  return [
    ...
    "For child_process.fork role-specific rows, reopen the exact live official node:child_process or child_process fork binding and preserve all four boundaries: modulePath selects executable child-module code, options.execPath selects the executable that creates the child, options.execArgv supplies Node runtime options before that module, and ordinary args are module data. For mcp-tool-fork-module-path, require tool input in argument zero, repeat the recorded fork:modulePath symbol in both validation and attackPath, report node-mcp-tool-untrusted-module-load with CWE-829, and use a fixed or explicit allowlisted child module as the repair; do not claim CWE-94, arbitrary code placement, or code execution beyond an existing selected module unless a separate chain proves it. For mcp-tool-fork-exec-path, require tool input in the exact object-literal options.execPath property, repeat the recorded fork:options.execPath symbol in both fields, report CWE-78 without CWE-88, and compare fixed process.execPath while keeping the same value only in ordinary child arguments. Reject replaced fork bindings or namespace members, dynamic or aliased option objects, spreads, computed or duplicate properties, unsupported overloads, and input confined to a different fork role. Validate module selection only with a checked-in fixture-local benign child selected by its inert path; validate execPath only with the current process.execPath. Never create or select an external attacker module, arbitrary executable, loader, inspector, shell, filesystem writer, network client, credential, persistence mechanism, or attacker code.",

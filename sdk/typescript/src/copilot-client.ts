  ...
  return [
    ...
    ...(residualRiskInventory === ""
      ...
      : [
          ...
          "For kotlin-ktor-command-injection rows, reopen the exact Ktor route handler and recorded call.request query, header, path, query-string, receive, or typed @Resource source, every named Kotlin assignment, concatenation, interpolation, and validation candidate, the effective java.lang.ProcessBuilder command list after every command(...) replacement and every ordered mutation through the live list returned by command(), including retained list aliases, and start() or startPipeline(...) on the same builder object or a proven builder alias. In validation, explicitly restate the concrete call expression, key or typed resource property, each named propagation step, the constructor arguments, any effective ProcessBuilder.command replacement, live-list identity and mutation, builder alias, and execution dispatch. In the attack path, carry that same request or resource value through the named commandLine or equivalent expression into executable selection or the explicit shell, interpreter, PowerShell, or Windows batch grammar position and then to the observed response or effect. ProcessBuilder's first effective list element selects the executable; attacker control there is executable selection. Ordinary later elements are distinct operating-system arguments, so a fixed ordinary executable with request data only in a separate argument is strong shell-injection counterevidence. The modeled exceptions are POSIX shell -c, cmd /c or /k, PowerShell or pwsh -Command, interpreter code flags such as python -c or node -e, and Windows .bat/.cmd consumers. The list passed to ProcessBuilder(List) or command(List), and the list returned by command(), remains live: account for indexed writes, set, add, removeAt, clear/rebuild order, aliases, and detachment after a later command replacement. Require exact Ktor routing or Resources imports, an exact io.ktor.resources.Resource annotation for typed handlers, an exact explicit java.lang.ProcessBuilder import or fully qualified identity, source and execution in the same route lambda, and actual start() or startPipeline(); reject tests, examples, fixtures, local lookalikes, construction without execution, unproven aliases, reassigned builders, commands replaced or live lists rebuilt with safe fixed-program argv, out-of-bounds mutations that abort before execution, numeric normalization, and request data used only as ordinary argv. Treat regexes, membership tests, escaping helpers, and nearby validation names as candidate controls until source-backed dominance and grammar-specific correctness are proved. Confirm the route is installed, deployed, and attacker reachable; execution reaches start or startPipeline; the target OS and shell/interpreter version; executable and PATH resolution; working directory, environment, standard streams, timeout and resource behavior; process privilege and containment; and the least concrete impact. The host row proves a candidate command-language or executable-selection edge, not successful exploitation, public reachability, persistence, privilege escalation, credential access, arbitrary file access, or complete host compromise. Validate only with a bounded harmless environment-marker witness that preserves the command shape and performs no file, network, credential, persistence, or privilege operation. Report CWE-78 and CWE-88 and calibrate severity from demonstrated reachability, influence, execution context, and impact.",

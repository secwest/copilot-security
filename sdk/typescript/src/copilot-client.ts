  ...
  return [
    ...
    ...(residualRiskInventory === ""
      ...
      : [
          ...
          "For node-mcp-tool-code-injection construction-lifecycle rows, require the recorded live global Function, exact vm.compileFunction, new vm.Script, or new vm.SourceTextModule construction and the exact same non-reassigned result's execution. A compiled Function must be invoked directly or through its live call/apply method; a Script must reach runInContext, runInNewContext, or runInThisContext; a SourceTextModule must complete an awaited legacy link before evaluate or modern linkRequests then instantiate then evaluate. Reject bare construction, unawaited legacy linking, incomplete module lifecycles, lookalikes, and replaced imports, constructor members, compiled values, prototype methods, or instance methods. Name construction and execution separately in validation and attackPath. Node documents that node:vm is not a security mechanism, while vm.Script construction compiles without running and vm.Module use has distinct creation, linking, and evaluation stages. Validate only with fixed side-effect-free arithmetic and the topology-matched fixed-grammar parser control.",

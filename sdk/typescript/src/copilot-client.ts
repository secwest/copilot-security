  ...
  return [
    ...
    ...(residualRiskInventory === ""
      ...
      : [
          ...
          "For cloudformation-public-admin-role rows, reopen the exact CloudFormation template, AWS::IAM::Role logical ID and optional RoleName, AssumeRolePolicyDocument statement, wildcard Principal, sts:AssumeRole action, and administrator permission attachment on that same role. The host emits this row only when an unrestricted Allow trust statement names Principal '*' or Principal.AWS containing '*', no effective trust Condition restricts it, and the same role receives the exact AWS AdministratorAccess managed policy or an unrestricted inline Allow statement with Action '*' and Resource '*'. An absent PermissionsBoundary or the exact AdministratorAccess boundary remains unbounded; other static or dynamic boundaries defer this model. A public trust, AdministratorAccess occurrence, wildcard action, or wildcard resource alone is not the joined chain. Verify the template is selected, transformed, deployed in the relevant account and partition, and not changed by parameters, macros, StackSets, nested stacks, CDK/SAM synthesis, overlays, drift, or a later policy update. An applicable nonempty trust Condition, a genuinely restrictive role PermissionsBoundary, a specific principal, removal of sts:AssumeRole, replacement of the administrator policy, stack exclusion, or deployment failure is strong counterevidence for this exact model; inspect the actual condition and boundary rather than assuming every declaration is effective. Establish the reachable principal independently: same-account identities may need no separate identity allow under role trust semantics, while cross-account callers normally need their own sts:AssumeRole permission; organization SCPs, session policies, explicit denies, region/account scope, maximum session duration, MFA or external-ID requirements, and current role existence can further constrain impact. Do not infer anonymous internet access, valid AWS credentials, successful AssumeRole, a specific account, or active deployment from static YAML or JSON alone. If deployed as modeled, prove the least concrete unauthorized secret, IAM, data, workload, billing, persistence, or destructive action available through the resulting administrator session and report CWE-269/CWE-284 without overstating cross-account or organization-wide compromise.",

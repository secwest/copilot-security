  ...
  return [
    ...
    ...(residualRiskInventory === ""
      ...
      : [
          ...
          "For java-file-getname-path-boundary rows, prove exact java.io.File identity, exact request-to-getName-to-sink value flow, and the sink's filesystem effect. File.getName preserves the exact parent component .. and therefore does not sanitize path traversal. Treat only fixed server-owned selection, a strict server-owned basename allowlist, or fail-closed rejection of the same reduced parent value before the sink as strong counterevidence; another value, logging, a substring check, or a check after the operation is insufficient. Then separately inspect link, junction, mount, writable-directory, race, authorization, decoding, separator, and platform behavior.",
          'For java-path-getfilename-path-boundary rows, prove exact java.nio.file.Path or Paths identity, exact request-to-Path-to-getFileName-to-sink value flow, and the sink\'s filesystem effect. A single-type import is authoritative over a same-package top-level lookalike; a wildcard import is not. Unqualified statically imported of/get calls require exact JDK ownership and no local or competing static method binding. Path.getFileName preserves the exact parent name .. and therefore does not sanitize traversal; a bare getFileName or getNameCount occurrence is not a control. Treat only fixed server-owned selection, a strict server-owned name allowlist, or fail-closed equality of the same reduced Path with exact Path.of("..") or Paths.get("..") before the sink as strong counterevidence. Another reduction, logging, substring matching, or a post-operation check is insufficient. Then separately inspect provider, null, link, junction, mount, writable-directory, race, authorization, decoding, separator, volume, and platform behavior.',

  ...
  return [
    ...
    ...(residualRiskInventory === ""
      ...
      : [
          ...
          "For node-mcp-tool-command-injection, node-mcp-tool-argument-injection, node-mcp-tool-path-traversal, and node-mcp-tool-ssrf rows, reopen the exact official MCP TypeScript SDK import, McpServer construction, live server alias, schema-bearing tool registration, callback input binding, every recorded local assignment or same-file helper call, and the exact process, filesystem, or network sink. When a row records an mcp-tool-helper-call, both validation and attackPath must repeat the exact helper symbol or explicitly call it the same-file helper/helper call; evidence in only one field is insufficient. Stable v2 registerTool callbacks receive tool input only when inputSchema is present; a schema-less callback receives context, while legacy v1 variadic server.tool uses its explicit schema before the callback. In both validation and attackPath, explicitly name the MCP client, model, tool caller, or tool invocation that can supply the callback input, and name inputSchema or schema validation plus the exact property the schema proves and the security property it does not prove. Schema validation proves shape, not that an arbitrary string is safe shell grammar, an executable name or interpreter option, a filesystem path, or a network destination. For command rows, distinguish child_process exec/execSync shell strings, shell-enabled spawn/execFile, fixed shell interpreter arguments, and attacker-selected executables from fixed executable plus separate argv data. Fixed executable plus argv is strong shell-injection counterevidence, but when the executable is process.execPath, explicitly verify that an exact -- end-of-options element precedes every untrusted argv element; without it, a dash-prefixed value remains in the Node option region as an interpreter option and is argument injection. In both validation and attackPath, use option region, end-of-options, or argument injection wording rather than relying on the bare -- token to explain this boundary. For path rows, prove that tool input reaches a path-bearing Node fs or fs/promises argument rather than file contents, encoding, flags, or callback data. path.join or path.resolve alone normalizes syntax but does not prove confinement; require an exact allowlist or a boundary-safe canonical containment design that accounts for absolute paths, separators, symlinks, and check/use races. A fixed operator path with tool input confined to write contents is strong counterevidence. For SSRF rows, prove that tool input selects the URL, host, hostname, protocol, or Axios baseURL rather than merely a request body or same-host path; a literal operator-pinned destination with input confined to body data is strong counterevidence. Reject local SDK/process/fs/network lookalikes, comments and strings, tests and generated files, schema-less v2 context callbacks, inert registrations, overwritten taint, unrelated helper parameters, fixed paths or destinations, and unsupported callback forms. Confirm the server is launched, transport and authentication policy, which client or model can invoke the tool, schema and authorization controls, process privilege, filesystem permissions and confinement, network egress, redirect and DNS behavior, destination parsing, timeouts and response-size limits, and the least concrete effect. A host row proves a candidate tool-input edge, not anonymous Internet reachability, successful command execution, arbitrary host-file access, metadata access, credential disclosure, persistence, or full host compromise. Validate command or argument behavior only with bounded inert fixed-string argv; validate filesystem behavior only inside a newly created disposable temporary tree containing synthetic marker data; validate network behavior only with a disposable loopback endpoint and fake data when stronger proof is necessary. Report CWE-78/CWE-88 for shell-command rows, CWE-88/CWE-94 for Node interpreter-option rows, CWE-22/CWE-73 for path rows, and CWE-918 for SSRF rows.",

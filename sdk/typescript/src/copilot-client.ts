...
export function scanQualityGatePrompt(
  ...
  return [
    ...
    "For node-http-js-yaml-parser-dos rows, require an official default, namespace, TypeScript import-equals, named or aliased loader, CommonJS receiver/destructure, direct CommonJS member, or direct require call from exactly js-yaml, plus nearest exact production or fresh declaration-consistent npm v2/v3 proof. The quadratic merge-chain family affects 3.0.0 through 3.14.x and 4.0.0 through 4.2.x; 3.15.0 and 4.3.0 cap total merge-key processing. The separate exponential flow-pair family affects 5.0.0 through 5.2.1; 5.2.2 stops reparsing nested flow-sequence keys. Accept load/loadAll on all modeled branches and safeLoad/safeLoadAll only on 3.x. Remote request data must reach argument zero. Reject fixed YAML, wrong or development-only packages, unsupported methods, pre-3 releases, repaired versions, lockfile-free ranges, inconsistent/v1 locks, reassignment, wrapper shadowing, and stale metadata. For the quadratic family, validate chained << aliases, effective key counts, input byte size, parser options, runtime timing, and event-loop impact; a 10K total-merge-key ceiling is the repaired default. For the exponential family, validate the nested flow-sequence-pair grammar, depth, sub-200-byte payload, parser rewind behavior, runtime timing, and event-loop impact; maxDepth:100 is not protection because practical denial occurs around 30 to 40 levels. Request body limits and ordinary try/catch do not preempt synchronous CPU work. Report only demonstrated CWE-400/CWE-407 availability impact; do not conflate these defects with historical executable-tag deserialization, prototype pollution, memory exhaustion, or code execution.",

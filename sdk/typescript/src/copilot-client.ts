...
export function scanQualityGatePrompt(
  ...
  return [
    ...
    "For traefik-replacepathregex-auth-bypass rows, reopen the exact Docker Compose Traefik image, file-provider command and mounted source/target/mode, public and protected router rules, entry points, middleware lists, shared backend service, ReplacePathRegex expression/replacement, and concrete authentication middleware. GHSA-cxjq-mrr5-89rv affects stable Traefik v2 through 2.11.51, v3.0.0 through 3.6.22, and v3.7.0 through 3.7.6; repairs are 2.11.52, 3.6.23, and 3.7.7. Require a public PathPrefix without a trailing separator, exactly one separator-free prefix rewrite such as ^/api(.*) to /$1, an authenticated sibling PathPrefix on the same entry point and defined backend service, an exact affected official traefik image, and a mounted dynamic file selected by --providers.file.filename. Accept short or long Compose bind syntax and read-only, explicit read-write, or default mount modes because exploitability does not depend on mount writability. Reject image membership alone, prereleases and repaired tags, digestless guesses, unmatched mounts/providers, traversal mounts, duplicate or aliased YAML, a mandatory separator capture, a non-traversal-producing replacement, public authentication, inert/empty auth configuration, different services or entry points, undefined services, and unrelated files. Validate with two loopback-only processes and an inert marker: the direct protected path must receive 401 or 403, the affected binary must forward the crafted /api../protected path so the disposable backend normalizes it to the protected marker, and the matching repaired branch must return 400 without reaching that backend path. Record image/version, provider and mount, both routers and priorities, entry point, middleware definitions, regex/replacement, raw and forwarded paths, direct/crafted statuses, backend hit booleans, repaired comparison, bindings, and cleanup. Report CWE-22 and only the demonstrated route-level authentication bypass and concrete protected backend effect. Do not infer arbitrary filesystem traversal, credential theft, administrative access, write impact, code execution, or host compromise without separate backend evidence; never use a real credential, protected service, or external listener.",

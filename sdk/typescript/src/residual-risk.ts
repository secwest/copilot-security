...
const FRAMEWORK_DATAFLOW_MODELS: readonly FrameworkDataflowModel[] = [
  ...
  {
    id: "node-http-prototype-pollution",
    language: "javascript-typescript",
    extensions: JAVASCRIPT_EXTENSIONS,
    activation: [/\[[^\]\r\n]+\]\s*\[[^\]\r\n]+\]\s*=(?!=|>)/u],
    sources: [
  ...
  {
    id: "node-opcua-server-nonce-cache-dos",
    language: "javascript-typescript",
    extensions: JAVASCRIPT_EXTENSIONS,
...
interface NodePrototypePollutionSink {
  keyExpressions: string[];
}

function nodePrototypePollutionSink(
  lines: readonly string[],
  line: number,
): NodePrototypePollutionSink | undefined {
  const original = javascriptCodeBeforeComment(lines[line - 1] ?? "");
  const structural = javascriptStructuralLines([original])[0] ?? "";
  const assignment =
    /\b([A-Za-z_$][\w$]*(?:\s*(?:\.\s*[A-Za-z_$][\w$]*|\[[^\]\r\n]+\]))+)\s*=(?!=|>)/gu;
  for (const candidate of structural.matchAll(assignment)) {
    if (candidate.index === undefined) continue;
    const leftStructural = candidate[1]!;
    const leftOriginal = original.slice(
      candidate.index,
      candidate.index + leftStructural.length,
    );
    const keyExpressions: string[] = [];
    const bracket = /\[([^\]\r\n]+)\]/gu;
    for (const match of leftStructural.matchAll(bracket)) {
      if (match.index === undefined) continue;
      const open = leftStructural.indexOf("[", match.index);
      const close = leftStructural.indexOf("]", open + 1);
      if (open < 0 || close < 0) continue;
      const expression = leftOriginal.slice(open + 1, close).trim();
      if (
        expression === "" ||
        /^(?:[+-]?\d+(?:\.\d+)?|["'`][\s\S]*["'`])$/u.test(expression)
      ) {
        continue;
      }
      keyExpressions.push(expression);
    }
    if (keyExpressions.length >= 2) return { keyExpressions };
  }
  return undefined;
}
...
function nodeMergeDeepVersionIsPrototypePollutionVulnerable(
  version: string,
): boolean {
  const parts = version.split(".").map(Number);
  if (parts.length !== 3 || parts.some((part) => !Number.isSafeInteger(part))) {
...
function nodeDeepExtendVersionIsPrototypePollutionVulnerable(
  version: string,
): boolean {
  const parts = version.split(".").map(Number);
  if (parts.length !== 3 || parts.some((part) => !Number.isSafeInteger(part))) {
...
function nodeNodeExtendVersionIsPrototypePollutionVulnerable(
  version: string,
): boolean {
  const parts = version.split(".").map(Number);
  if (parts.length !== 3 || parts.some((part) => !Number.isSafeInteger(part))) {
  ...
  const [major, minor, patch] = parts as [number, number, number];
  const vulnerableLegacyLine =
    major < 1 || (major === 1 && (minor < 1 || (minor === 1 && patch < 7)));
  const vulnerableTwoLine = major === 2 && minor === 0 && patch === 0;
...
function nodePrototypeMergeVersionIsVulnerable(
  ...
  switch (dependencyName) {
    case "lodash":
      return nodeLodashVersionIsPrototypePollutionVulnerable(version);
    case "lodash.merge":
      return nodeLodashMergePackageVersionIsPrototypePollutionVulnerable(
        version,
      );
    case "merge-deep":
      return nodeMergeDeepVersionIsPrototypePollutionVulnerable(version);
    case "extend":
...
function nodePrototypeMergeSinkKind(
  dependencyName: NodePrototypeMergeDependencyName,
  proof: NodeRuntimeDependency["proof"],
): NodePrototypeMergeSink["kind"] {
  const lockResolved = proof === "npm-lockfile";
  switch (dependencyName) {
    case "lodash":
      return lockResolved
        ? "lock-resolved-vulnerable-lodash-recursive-merge"
...
function nodePrototypeMergeSink(
  ...
  for (let index = 0; index < codeLines.length; index += 1) {
    ...
    const mergeDeepImport =
      /^\s*import\s+([A-Za-z_$][\w$]*)\s+from\s+["']merge-deep["']/u.exec(
        structural,
      );
    ...
    const deepExtendImport =
      /^\s*import\s+([A-Za-z_$][\w$]*)\s+from\s+["']deep-extend["']/u.exec(
        structural,
      );
    const deepExtendRequire =
    ...
    const nodeExtendImport =
      /^\s*import\s+([A-Za-z_$][\w$]*)\s+from\s+["']node\.extend["']/u.exec(
        structural,
      );
    const nodeExtendRequire =
    ...
    const mergeDeepDirect = mergeDeepImport?.[1] ?? mergeDeepRequire?.[1];
    if (mergeDeepDirect !== undefined) {
      bindings.push({
        kind: "direct",
        local: mergeDeepDirect,
        line: index + 1,
    ...
    const deepExtendDirect = deepExtendImport?.[1] ?? deepExtendRequire?.[1];
    if (deepExtendDirect !== undefined) {
      bindings.push({
        kind: "direct",
    ...
    const nodeExtendDirect = nodeExtendImport?.[1] ?? nodeExtendRequire?.[1];
    if (nodeExtendDirect !== undefined) {
      bindings.push({
        kind: "direct",
        local: nodeExtendDirect,
        line: index + 1,
  ...
  for (const binding of bindings) {
    ...
    if (
      ...
      !nodePrototypeMergeVersionIsVulnerable(
        binding.dependencyName,
        dependency.version,
      )
    ) {
    ...
    if (sourceExpressions.length > 0) {
      return {
        sourceExpressions,
        kind: nodePrototypeMergeSinkKind(
          binding.dependencyName,
          dependency.proof,
        ),
      };
...
type NodeOpcuaServerBinding = {
  local: string;
  line: number;
  member?: "OPCUAServer";
};

function nodeOpcuaVersionHasUnboundedNonceCache(version: string): boolean {
  const parts = version.split(".").map(Number);
  if (parts.length !== 3 || parts.some((part) => !Number.isSafeInteger(part))) {
    return false;
  }
  ...
  return (
    major < 2 ||
    (major === 2 && (minor < 165 || (minor === 165 && patch === 0)))
  );
}

function nodeOpcuaServerDosSink(
  files: readonly SourceFileSnapshot[],
  path: string,
  lines: readonly string[],
  line: number,
): NodeOpcuaServerDosSink | undefined {
  if (javascriptTestOrExamplePath(path)) return undefined;
  const dependency = nodeRuntimeDependency(files, path, "node-opcua");
  if (
    dependency === undefined ||
    !nodeOpcuaVersionHasUnboundedNonceCache(dependency.version)
  ) {
    return undefined;
  }
  const codeLines = javascriptCodeLinesWithoutComments(lines);
  ...
  for (const binding of bindings) {
    ...
    for (let index = line; index < structuralLines.length; index += 1) {
      ...
      const prefix =
        dependency.proof === "npm-lockfile" ? "lock-resolved-" : "";
      return {
        source: { kind: "untrusted-opcua-session-nonce", line: startLine },
        sinkLine: line,
        kind: `${prefix}vulnerable-node-opcua-unbounded-nonce-cache-dos`,
        dependency,
      };
    }
  }
    ...
    for (const sink of sinks) {
      ...
      const nodeMongooseBulkWrite =
        model.id === "node-http-mongoose-bulk-write"
          ...
          : undefined;
      const nodeMongooseAggregate =
        model.id === "node-http-mongoose-aggregate"
          ? nodeMongooseAggregateSink(lines, sink.line)
          : undefined;
      ...
      const nodeMongooseBulkWriteResolution =
        ...
        nodeMongooseBulkWrite !== undefined
          ...
          : undefined;
      const nodeMongooseAggregateResolution =
        model.id === "node-http-mongoose-aggregate" &&
        nodeMongooseAggregate !== undefined
      ...
      const aggregateSinkMetadata =
        nodeMongooseAggregateResolution === undefined
          ? undefined
...
interface NodeOpcuaExportedServer {
  dependency: NodeRuntimeDependency;
  file: SourceFileSnapshot;
  line: number;
  symbol: string;
...
function nodeOpcuaCrossFileServerDosRecords(
  files: readonly SourceFileSnapshot[],
): ResidualRiskRecord[] {
  const knownPaths = new Map(
    files.map((file) => [modelPathComparisonKey(file.path), file.path]),
  ...
  for (const caller of files) {
    ...
    for (const imported of importedJavascriptSymbols(caller.lines)) {
      ...
      for (
        ...
        const sinkKind = `${prefix}vulnerable-node-opcua-unbounded-nonce-cache-dos`;
        records.push({
          path: server.file.path,
          line: server.line,
          categories: [
            "framework-dataflow:node-opcua-server-nonce-cache-dos",
            "framework-cross-file-server-instance",
            "modeled-source:untrusted-opcua-session-nonce",
            `modeled-sink:${sinkKind}`,
          ],
          priority: 120,
          startLine: sinkStart,
          ...
          frameworkModel: {
            ...
            propagators: [
              ...
              },
              {
                ...
                symbol: `node-opcua@${server.dependency.version}:${server.dependency.proof}:unbounded-session-nonce-cache`,
              },
            ],
            candidateControls: [],
          },
    ...
    for (const model of FRAMEWORK_DATAFLOW_MODELS) {
      ...
      for (const wrapper of exportedFunctions) {
        for (const sink of sinks) {
          ...
          const nodeMongooseAggregate =
            model.id === "node-http-mongoose-aggregate"
              ? nodeMongooseAggregateSink(file.lines, sink.line)
              : undefined;
          if (model.id === "node-http-ssrf" && nodeHttpSink === undefined) {
            continue;
          }
...
interface NodeMongooseAggregateSink {
  pipelineExpression: string;
  positions: NodeMongooseAggregatePosition[];
}

interface NodeMongooseAggregateSinkMetadata {
  cweIds: string[];
  kind: string;
}
...
function nodeMongooseAggregatePositions(
  lines: readonly string[],
  pipelineExpression: string,
  sinkLine: number,
  ...
  const elements = javascriptArrayEntries(pipeline);
  ...
  for (const element of elements) {
    const stageObject = resolvedJavascriptExpression(
      lines,
      element.value,
      element.line,
    );
    const stageEntries = javascriptObjectEntries(stageObject);
    if (stageEntries.length === 0 || /\.\.\.|\[/u.test(stageObject.value)) {
      addPosition(
        stageObject.value.replace(/^\s*\.\.\./u, ""),
        stageObject.line,
        "dynamic",
      );
      continue;
...
function nodeMongooseAggregateIsConsumed(
  ...
  const windowLines = lines.slice(line - 1, Math.min(lines.length, line + 24));
  const structuralWindow = javascriptStructuralLines(windowLines).join("\n");
  const match = call.exec(structuralWindow);
  if (match === null) return false;
  const open = structuralWindow.indexOf("(", match.index);
  if (open < 0) return false;
  const close = matchingCallParenthesis(structuralWindow, open);
  if (close < 0) return false;
  const prefix = structuralWindow.slice(0, match.index);
  const tail = structuralWindow.slice(close + 1);
  const statementEnd = tail.indexOf(";");
  const statementTail = tail.slice(
    0,
    statementEnd >= 0 ? statementEnd : tail.length,
  );
  if (/\bawait\b/u.test(prefix)) return true;
  if (/\.\s*(?:exec|then|catch|finally|cursor)\s*\(/u.test(statementTail)) {
    return true;
  }
  if (
    /\breturn\b/u.test(prefix) &&
    wrapper !== undefined &&
    /\basync\b/u.test(lines[wrapper.startLine - 1] ?? "")
  ) {
    return true;
  ...
  if (assignment === null || wrapper === undefined) return false;
  const aggregate = assignment[1]!;
  if (
    javascriptIdentifierReassignedBetween(
      lines,
  ...
  const remainder = javascriptStructuralLines(
    lines.slice(line, wrapper.endLine),
  ).join("\n");
  return (
    new RegExp(
      `\\bawait\\s+${escapeRegularExpression(aggregate)}\\b|\\b${escapeRegularExpression(aggregate)}\\s*\\.\\s*(?:exec|then|catch|finally|cursor)\\s*\\(`,
      "u",
    ).test(remainder) &&
    !new RegExp(
      `\\b${escapeRegularExpression(aggregate)}\\s*\\.\\s*(?:pipeline|model)\\s*\\(`,
      "u",
    ).test(remainder)
  );
}

function nodeMongooseAggregateSink(
  lines: readonly string[],
  line: number,
): NodeMongooseAggregateSink | undefined {
  const context = nodeMongooseBindingContext(lines);
  const wrapper = context.wrappers.find(
    (candidate) => line >= candidate.startLine && line <= candidate.endLine,
  );
  ...
  for (const model of context.models) {
    ...
    const call = new RegExp(
      `\\b${escapeRegularExpression(model.local)}\\s*\\.\\s*aggregate\\s*\\(`,
      "u",
    );
    if (!call.test(structuralLine)) continue;
    if (!nodeMongooseAggregateIsConsumed(lines, line, call, wrapper)) continue;
    const arguments_ = javascriptCallArgumentsAtLine(lines, line, call);
...
function nodeMongooseAggregateSinkMetadata(
  position: NodeMongooseAggregatePosition,
): NodeMongooseAggregateSinkMetadata {
...
function fixedMongooseAggregateMatchCoversParameter(
  matchExpression: string,
  parameter: string,
): boolean {
  const code = javascriptCodeLinesWithoutComments(
    matchExpression.split(/\r?\n/u),
  ).join("\n");
  if (/\.\.\.|\[/u.test(code)) return false;
  const properties = javascriptObjectEntries({ line: 1, value: code });
  if (properties.length === 0) return false;
  ...
  if (taintedProperties.length === 0) return false;
  const equalityOperand = new RegExp(
    `^\\s*${escapeRegularExpression(parameter)}(?:\\s*\\.\\s*[A-Za-z_$][\\w$]*)*\\s*$`,
    "u",
  );
  return taintedProperties.every(({ value }) => {
    const operators = javascriptObjectEntries({ line: 1, value });
    return (
      operators.length === 1 &&
      operators[0]?.key === "$eq" &&
      equalityOperand.test(operators[0].value)
    );
  });
}

function nodeMongooseAggregateControls(
  lines: readonly string[],
  sink: NodeMongooseAggregateSink,
  sinkLine: number,
): Array<{ kind: string; line: number }> {
  const context = nodeMongooseBindingContext(lines);
  const wrapper = context.wrappers.find(
  ...
  const controls: Array<{ kind: string; line: number }> = [];
  for (const position of sink.positions) {
    if (position.stage !== "$match") continue;
    const taintedParameters = (wrapper?.parameters ?? []).filter((parameter) =>
      lineReferencesIdentifier(position.expression, parameter),
    );
    if (
      taintedParameters.length > 0 &&
      taintedParameters.every((parameter) =>
        fixedMongooseAggregateMatchCoversParameter(
          position.expression,
          parameter,
        ),
      )
...
function compareResidualRiskRecords(
  ...
  const hasEstablishedFixedBoundary = (record: ResidualRiskRecord): boolean => {
    ...
    if (framework?.id === "node-http-mongoose-aggregate") {
      return framework.candidateControls.some(
        (control) => control.kind === "fixed-aggregate-match-value-boundary",
      );
    }
    return false;
  };

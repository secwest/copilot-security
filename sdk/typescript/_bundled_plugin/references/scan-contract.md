...
- `report.md`: deterministic readable projection sealed to the same completed scan
...
The existing `report.md` output remains a readable projection rather than a semantic source of truth, but its generated bytes are included in the completed-scan seal so consumers cannot display an independently modified narrative. Regenerate a changed report as a new scan instead of mutating a completed bundle. Generated exports such as SARIF remain unsealed downstream projections.
...
A sealed manifest records the completed timestamp and hashes for the canonical documents, deterministic `report.md`, and immutable evidence receipts included in that bundle. Later adapters may read the sealed bundle to create unsealed exports, but must not mutate the sealed manifest, canonical documents, or report. Store additional projections separately. Every sealed manifest includes exactly one artifact record for each canonical JSON document and `report.md`, and artifact paths must not repeat.

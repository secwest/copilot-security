...
- Note unique security considerations for the codebase, for example:
  ...
  - WebAuthn/passkey authentication trust across requested account, browser transaction, challenge freshness and one-time use, RP ID and origin, allowed credentials, credential registration and owner, user handle, authenticator flags and sign counter, signature verification, replay state, and final session principal; distinguish proof of credential possession from authorization to a separately selected account

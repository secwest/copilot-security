...
- Note unique security considerations for the codebase, for example:
  - authn/authz, session management, JWT/JWS/OIDC algorithm, remote-key, issuer-to-JWKS, key-cache, claim, nonce, and replay binding, SAML/federated signed-object and issuer/audience/recipient/replay binding, CSRF, XSS, SSRF, SQL/document-query operator injection, untrusted upload/content placement and its serving or execution roots, HTTP framing/parser agreement across proxies, gateways, protocol translators, and backends, bulk object binding and mass assignment, tenant boundaries, rate limits, and secret handling for web applications

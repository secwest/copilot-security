...
For infrastructure-as-code security configuration, distinguish the declared security defect from confirmation that a particular environment currently deploys it. An exact deployable resource whose source proves the complete unsafe trust, authority, exposure, or host-access chain can be `reportable` with deployment and attacker access stated as preconditions. Absence of cloud credentials, live cluster/account telemetry, a current resource identifier, or a deployment receipt from the repository is uncertainty that calibrates confidence and impact wording; it is not by itself counterevidence, a suppression reason, or proof that the resource is inactive. Suppress only when repository or runtime evidence proves the unsafe resource is excluded, rejected, replaced, or constrained by an effective control. For a CloudFormation IAM role that statically joins an unrestricted wildcard AWS principal and `sts:AssumeRole` with unbounded administrator permissions, an attacker-controlled external AWS account supplies a realistic conditional principal because its owner can grant caller-side `sts:AssumeRole`; do not require the target repository to contain the attacker's credentials. Serialize unchanged deployment and effective caller-side authorization as explicit preconditions and remaining uncertainties in the finding and attack path. Do not state that operating any AWS principal is sufficient, call it the only precondition, or describe session issuance and administrator impact as unconditional. A same-account exception requires evidence of the target account relationship; when that identity is absent, use the cross-account permission model. Do not turn that path into anonymous internet access or an active-session claim without separate evidence.

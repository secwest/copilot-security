...
describe("Go HTTP object-authorization framework model", () => {
  test("keeps the exploit and authorization control under perfect benchmark gates", async () => {
    ...
    expect(manifest.cases.map(({ id }) => id)).toEqual([
      ...
      "go-cross-package-imported-helper-switch-write-delete-idor",
      "go-cross-package-safe-imported-helper-switch-write-authorization",
      "go-cross-package-imported-helper-expressionless-switch-write-delete-idor",
      ...
      "go-cross-package-imported-helper-aliased-type-switch-write-delete-idor",
      "go-cross-package-safe-imported-helper-aliased-type-switch-write-authorization",
      "go-cross-package-imported-helper-empty-interface-type-switch-write-delete-idor",
      ...
      "go-cross-package-promoted-method-delete-idor",
      "go-cross-package-safe-promoted-method-authorization",
    ]);
    ...
    expect(manifest.cases[48]?.expected).toHaveLength(1);
    expect(manifest.cases[49]?.expected).toEqual([]);
    expect(manifest.cases[50]?.expected).toHaveLength(1);
    ...
    expect(manifest.cases[56]?.expected).toHaveLength(1);
    expect(manifest.cases[57]?.expected).toEqual([]);
    expect(manifest.cases[58]?.expected).toHaveLength(1);
    ...
    expect(manifest.cases[68]?.expected).toHaveLength(1);
    expect(manifest.cases[69]?.expected).toEqual([]);
  });
  ...
  test("preserves imported helper switch writes and their control", async () => {
    const vulnerable = await fixtureInventory(
      "go-cross-package-imported-helper-switch-write-delete-idor",
    );
    const safe = await fixtureInventory(
  ...
  test("preserves imported helper expressionless switch writes and their control", async () => {
    const vulnerable = await fixtureInventory(
      "go-cross-package-imported-helper-expressionless-switch-write-delete-idor",
    );
    const safe = await fixtureInventory(
  ...
  test("preserves imported helper initialized switch writes and their control", async () => {
    const vulnerable = await fixtureInventory(
      "go-cross-package-imported-helper-initialized-switch-write-delete-idor",
    );
    const safe = await fixtureInventory(
  ...
  test("preserves imported helper aliased type switch writes and their control", async () => {
    const vulnerable = await fixtureInventory(
      "go-cross-package-imported-helper-aliased-type-switch-write-delete-idor",
    );
    const safe = await fixtureInventory(
  ...
  test("preserves imported helper empty-interface type switch writes and their control", async () => {
    const vulnerable = await fixtureInventory(
      "go-cross-package-imported-helper-empty-interface-type-switch-write-delete-idor",
    );
    const safe = await fixtureInventory(
  ...
  test("preserves imported helper named-interface type switch writes and their control", async () => {
    const vulnerable = await fixtureInventory(
      "go-cross-package-imported-helper-named-interface-type-switch-write-delete-idor",
    );
    const safe = await fixtureInventory(
    ...
    const propagators = vulnerable[0]?.frameworkModel?.propagators ?? [];
    expect(propagators).toHaveLength(28);
    expect(propagators).toEqual(
      expect.arrayContaining([
        expect.objectContaining({
          kind: "go-method-receiver-constructor-helper-call",
  ...
  test("preserves promoted concrete methods and their authorization control", async () => {
    const vulnerable = await fixtureInventory(
      "go-cross-package-promoted-method-delete-idor",
    );
    const safe = await fixtureInventory(
    ...
    expect(
      vulnerable[0]?.frameworkModel?.propagators.filter(
        ({ kind }) => kind === "go-method-receiver-promoted-field",
      ),
    ).toEqual([
      expect.objectContaining({
        path: "internal/service/service.go",
        symbol: "Service.RepositoryLayer:RepositoryLayer",
      }),
      expect.objectContaining({
        path: "internal/service/service.go",
        symbol: "RepositoryLayer.Store:*store.Store",
      }),
    ]);
    expect(safe).toHaveLength(1);
    expect(safe[0]).toMatchObject({
...
func Handler(db *sql.DB, w http.ResponseWriter, r *http.Request) {
  ...
  return service
    ...
    const switchWrites = switched[0]?.frameworkModel?.propagators.filter(
      ({ kind, symbol }) =>
        kind === "go-method-receiver-constructor-field-write" &&
        symbol === "Layer.repository:invoices.Store",
    );
    expect(switchWrites).toHaveLength(3);
    expect(new Set(switchWrites?.map(({ line }) => line)).size).toBe(3);
    const expressionlessSwitched =
      await repository(`func NewService(repository InvoiceRepository, label string) *Service {
  service := &Service{layer: &Layer{label: label}}
  primary := service
  secondary := service
  ...
  return service
    ...
    expect(expressionlessSwitched).toHaveLength(1);
    const expressionlessSwitchWrites =
      expressionlessSwitched[0]?.frameworkModel?.propagators.filter(
        ({ kind, symbol }) =>
          kind === "go-method-receiver-constructor-field-write" &&
          symbol === "Layer.repository:invoices.Store",
      );
    expect(expressionlessSwitchWrites).toHaveLength(3);
    expect(
      new Set(expressionlessSwitchWrites?.map(({ line }) => line)).size,
    ).toBe(3);
    expect(
      await repository(`func NewService(repository InvoiceRepository, label string) *Service {
  service := &Service{layer: &Layer{label: label}}
  ...
  return service
    ...
    expect(initializedSwitched).toHaveLength(1);
    const initializedSwitchWrites =
      initializedSwitched[0]?.frameworkModel?.propagators.filter(
        ({ kind, symbol }) =>
          kind === "go-method-receiver-constructor-field-write" &&
          symbol === "Layer.repository:invoices.Store",
      );
    expect(initializedSwitchWrites).toHaveLength(3);
  ...
  return service
    ...
    expect(aliasedTypeSwitched).toHaveLength(1);
    expect(
      await repository(`func NewService(repository InvoiceRepository, label string) *Service {
  service := &Service{layer: &Layer{label: label}}
  alias1 := repository
  ...
  return service
    ...
    const convertedAliasTypeSwitched =
      await repository(`func NewService(repository InvoiceRepository, label string) *Service {
  service := &Service{layer: &Layer{label: label}}
  candidate := any(repository)
  switch candidate.(type) {
  case *Store:
    service.layer.repository = repository
  default:
  ...
  return service
    ...
    expect(convertedAliasTypeSwitched).toHaveLength(1);
    expect(
      await repository(`func NewService(repository InvoiceRepository, label string) *Service {
  service := &Service{layer: &Layer{label: label}}
  candidate := service.layer.repository
  switch candidate.(type) {
  case *Store:
    service.layer.repository = repository
  default:
  ...
  return service
    ...
    expect(
      await repository(`func NewService(repository InvoiceRepository, label string) *Service {
  service := &Service{layer: &Layer{label: label}}
  candidate, chosen := repository, true
  _ = chosen
  switch candidate.(type) {
  case *Store:
    service.layer.repository = repository
  ...
  return service
    ...
    expect(
      await repository(`func NewService(repository InvoiceRepository, label string) *Service {
  service := &Service{layer: &Layer{label: label}}
  var candidate InvoiceRepository
  if label == "primary" {
    candidate = repository
  } else {
    candidate = repository
  ...
  return service
    ...
    expect(
      await repository(`func NewService(repository InvoiceRepository, label string) *Service {
  service := &Service{layer: &Layer{label: label}}
  switch selected := repository; selected {
  case nil:
    service.layer.repository = repository
  default:
    service.layer.repository = repository
  ...
  return service
    ...
    const convertedTypeSwitched =
      await repository(`func NewService(repository InvoiceRepository, label string) *Service {
  service := &Service{layer: &Layer{label: label}}
  switch any(repository).(type) {
  case *Store:
    service.layer.repository = repository
  default:
    service.layer.repository = repository
  ...
  return service
    ...
    expect(convertedTypeSwitched).toHaveLength(1);
    expect(
      await repository(`func NewService(repository InvoiceRepository, label string) *Service {
  service := &Service{layer: &Layer{label: label}}
  candidate := repository
  switch selected := interface { }(candidate).(type) {
  case *Store:
    _ = selected
    service.layer.repository = repository
  default:
  ...
  switch selected := candidate.(type) {
  case *Store:
    _ = selected
    service.layer.repository = repository
  default:
  ...
  return service
    ...
    expect(
      await repository(`func NewService(repository InvoiceRepository, label string) *Service {
  service := &Service{layer: &Layer{label: label}}
  switch InvoiceRepository(repository).(type) {
  case *Store:
    service.layer.repository = repository
  default:
    service.layer.repository = repository
  ...
  return service
    ...
    expect(
      await repository(`type DeleteRepository interface { DeleteInvoice(context.Context, *sql.DB, string) error }
func NewService(repository InvoiceRepository, label string) *Service {
  service := &Service{layer: &Layer{label: label}}
  candidate := DeleteRepository(repository)
  switch candidate.(type) {
  case *Store:
    service.layer.repository = repository
  default:
  ...
  return service
    ...
    expect(
      await repository(`type DynamicRepository interface {}
func NewService(repository InvoiceRepository, label string) *Service {
  service := &Service{layer: &Layer{label: label}}
  switch DynamicRepository(repository).(type) {
  case *Store:
    service.layer.repository = repository
  default:
    service.layer.repository = repository
  ...
  return service
    ...
    expect(
      await repository(
        `func NewService(repository InvoiceRepository, label string) *Service {
  service := &Service{layer: &Layer{label: label}}
  switch contracts.DynamicRepository(repository).(type) {
  case *Store:
    service.layer.repository = repository
  default:
    service.layer.repository = repository
  ...
  return service
    ...
    expect(
      await repository(
        `func NewService(repository contracts.InvoiceRepository, label string) *Service {
  service := &Service{layer: &Layer{label: label}}
  switch contracts.InvoiceRepository(repository).(type) {
  case *Store:
    service.layer.repository = repository
  default:
    service.layer.repository = repository
...
type InvoiceRepository interface { DeleteInvoice(context.Context, *sql.DB, string) error }`,
    ...
    expect(
      await repository(
        `func NewService(repository InvoiceRepository, label string) *Service {
  service := &Service{layer: &Layer{label: label}}
  contracts := func(InvoiceRepository) interface{} { return &OtherStore{} }
  switch contracts.DynamicRepository(repository).(type) {
  case *Store:
    service.layer.repository = repository
  default:
...
func NewService(repository InvoiceRepository, label string) *Service {
  ...
  alias6 := alias5
  alias7 := alias6
  candidate := DeleteRepository(alias7)
  switch candidate.(type) {
  case *Store:
    service.layer.repository = repository
  default:
...
func NewService(repository InvoiceRepository, label string) *Service {
  ...
  alias7 := alias6
  alias8 := alias7
  candidate := DeleteRepository(alias8)
  switch candidate.(type) {
  case *Store:
    service.layer.repository = repository
  default:
  ...
  return service
    ...
    expect(
      await repository(`type BroaderRepository interface {
  DeleteInvoice(context.Context, *sql.DB, string) error
  ArchiveInvoice(context.Context, *sql.DB, string) error
}
func NewService(repository InvoiceRepository, label string) *Service {
  service := &Service{layer: &Layer{label: label}}
  switch BroaderRepository(repository).(type) {
  case *Store:
    service.layer.repository = repository
  default:
    service.layer.repository = repository
  ...
  return service
    ...
    expect(
      await repository(`type MismatchedRepository interface { DeleteInvoice(context.Context, *sql.DB, int) error }
func NewService(repository InvoiceRepository, label string) *Service {
  service := &Service{layer: &Layer{label: label}}
  switch MismatchedRepository(repository).(type) {
  case *Store:
    service.layer.repository = repository
  default:
    service.layer.repository = repository
  ...
  return service
    ...
    expect(
      await repository(`type DeleteRepository interface { DeleteInvoice(context.Context, *sql.DB, string) error }
func choose(repository InvoiceRepository) InvoiceRepository { return repository }
func NewService(repository InvoiceRepository, label string) *Service {
  service := &Service{layer: &Layer{label: label}}
  switch DeleteRepository(choose(repository)).(type) {
  case *Store:
    service.layer.repository = repository
  default:
    service.layer.repository = repository
...
func NewService(repository InvoiceRepository, label string) *Service {
  service := &Service{layer: &Layer{label: label}}
  DeleteRepository := func(InvoiceRepository) interface{} { return &OtherStore{} }
  switch DeleteRepository(repository).(type) {
  case *Store:
    service.layer.repository = repository
  default:
    service.layer.repository = repository
  ...
  return service
    ...
    expect(
      await repository(
        `func NewService(repository InvoiceRepository, label string) *Service {
  service := &Service{layer: &Layer{label: label}}
  switch contracts.DeleteRepository(repository).(type) {
  case *Store:
    service.layer.repository = repository
  default:
    service.layer.repository = repository
...
func NewService(repository InvoiceRepository, label string) *Service {
  service := &Service{layer: &Layer{label: label}}
  switch any(choose(repository)).(type) {
  case *Store:
    service.layer.repository = repository
  default:
    service.layer.repository = repository
  ...
  return service
    ...
    expect(
      await repository(`func NewService(repository InvoiceRepository, label string) *Service {
  service := &Service{layer: &Layer{label: label}}
  any := func(InvoiceRepository) interface{} { return &OtherStore{} }
  switch any(repository).(type) {
  case *Store:
    service.layer.repository = repository
  default:
  ...
  return service
    ...
    const packageAny = {
      "shadow.go": `package invoices
func any(InvoiceRepository) interface{} { return &OtherStore{} }`,
    };
    expect(
      await repository(
        `func NewService(repository InvoiceRepository, label string) *Service {
  ...
  return service
        ...
        {
          "shadow.go": `package invoices
var (placeholder int; any = placeholder)`,
        },
      ),
    ).toEqual([]);
    expect(
  ...
  return service
        ...
        {
          "go.mod": "module example.com/invoices\n\ngo 1.26\n",
          "helper/placeholder.go": "package helper",
          "shadow/placeholder.go": "package shadow",
        },
        'helper "example.com/invoices/helper"; any "example.com/invoices/shadow"',
      ),
    ).toEqual([]);
    expect(
      await repository(
  ...
  return service
}`,
        {
          "go.mod": "module example.com/invoices\n\ngo 1.26\n",
          "helper/placeholder.go": "package helper",
          "helper/placeholder_test.go": "package helper_test",
        },
        '"example.com/invoices/helper"',
      ),
    ).toHaveLength(1);
    expect(
      await repository(
        `func NewService(repository InvoiceRepository, label string) *Service {
  service := &Service{layer: &Layer{label: label}}
  switch interface{}(repository).(type) {
  case *Store:
    service.layer.repository = repository
  default:
    service.layer.repository = repository
  ...
  return service
        ...
        'any "example.com/invoices/shadow"',
      ),
    ).toEqual([]);
    expect(
      await repository(
  ...
  return service
        ...
        {
          "go.mod": "module example.com/invoices\n\ngo 1.26\n",
          "any/placeholder.go": "package any",
        },
        '"example.com/invoices/any"',
      ),
    ).toEqual([]);
    expect(
      await repository(`func NewService(repository InvoiceRepository, label string) *Service {
...
func Handler(db *sql.DB, w http.ResponseWriter, r *http.Request) {
  ...
  service.DeleteInvoice(r.Context(), db, invoiceID)
    ...
    const switched = await scan(`  switch label {
  case "primary":
    first.holder.repository = repository
  case "secondary":
    second.holder.repository = repository
  default:
    ...
    expect(switchWrites).toHaveLength(3);
    expect(new Set(switchWrites?.map(({ line }) => line)).size).toBe(3);
    const expressionlessSwitched = await scan(`  switch {
  case label == "primary":
    first.holder.repository = repository
    break
  case label == "secondary":
  ...
  default:
    ...
    const initializedSwitched =
      await scan(`  switch selected := label; selected {
  case "primary":
    first.holder.repository = repository
  case "secondary":
    second.holder.repository = repository
  ...
  default:
    ...
    expect(
      await scan(`  switch selected := choose(label); selected {
  case "primary":
    first.holder.repository = repository
  default:
    second.holder.repository = repository
    ...
    expect(
      await scan(`  switch first := label; first {
  case "primary":
    second.holder.repository = repository
  default:
    third.holder.repository = repository
...
${bindings}
  ...
  test("resolves exact nested promoted methods for concrete and interface dispatch", async () => {
    const shared = {
      "go.mod": `module example.com/promoted-methods
...
func (*Store) DeleteInvoice(ctx context.Context, db *sql.DB, invoiceID string) error {
  ...
  return err
    ...
    const concrete = await repositoryInventory({
      ...shared,
      "handler.go": `package invoices
import (
  "context"
  "database/sql"
  "net/http"
  store "example.com/promoted-methods/internal/store"
)
type Layer struct { *store.Store }
type Service struct { Layer }
func Handler(db *sql.DB, w http.ResponseWriter, r *http.Request) {
  invoiceID := r.PathValue("invoiceID")
  service := &Service{Layer: Layer{Store: &store.Store{}}}
  service.DeleteInvoice(r.Context(), db, invoiceID)
}`,
    });
    expect(concrete).toHaveLength(1);
    expect(concrete[0]?.path).toBe("internal/store/store.go");
    expect(
      concrete[0]?.frameworkModel?.propagators.filter(
        ({ kind }) => kind === "go-method-receiver-promoted-field",
      ),
    ).toEqual([
      expect.objectContaining({
      ...
      expect.objectContaining({
        path: "handler.go",
        symbol: "Layer.Store:*store.Store",
      }),
    ]);

    const interfaceDispatch = await repositoryInventory({
      ...shared,
      "handler.go": `package invoices
import (
  "context"
...
func Handler(db *sql.DB, w http.ResponseWriter, r *http.Request) {
  invoiceID := r.PathValue("invoiceID")
  var repository InvoiceRepository = Service{Store: &store.Store{}}
  repository.DeleteInvoice(r.Context(), db, invoiceID)
}`,
    });
    expect(interfaceDispatch).toHaveLength(1);
    expect(interfaceDispatch[0]?.path).toBe("internal/store/store.go");
    expect(
      interfaceDispatch[0]?.frameworkModel?.propagators.map(({ kind }) => kind),
    ).toContain("go-interface-receiver-binding");
    expect(
      interfaceDispatch[0]?.frameworkModel?.propagators.filter(
        ({ kind }) => kind === "go-method-receiver-promoted-field",
      ),
    ).toEqual([
      expect.objectContaining({ symbol: "Service.Store:*store.Store" }),
    ]);
  });

  test("applies promoted pointer method sets and shallow selector precedence exactly", async () => {
    const scan = async (
      declarations: string,
      setup: string,
      call = "service.DeleteInvoice(r.Context(), db, invoiceID)",
    ): Promise<ModelRecord[]> =>
      repositoryInventory({
        "invoices.go": `package invoices
import (
...
${setup}
  ${call}
    ...
    const addressableValue = await scan(
      "type Service struct { Store }",
      "  service := Service{}",
    );
    expect(addressableValue).toHaveLength(1);

    expect(
      await scan(
        "type Service struct { Store }",
        "  var service InvoiceRepository = Service{}",
      ),
    ).toEqual([]);

    const embeddedPointerInterface = await scan(
      "type Service struct { *Store }",
      "  var service InvoiceRepository = Service{Store: &Store{}}",
    );
    expect(embeddedPointerInterface).toHaveLength(1);
    expect(
      await scan(
        "type Service struct { *Store }",
        "  var service InvoiceRepository = Service{}",
    ...
    expect(
      await scan("type Service struct { *Store }", "  service := &Service{}"),
    ).toEqual([]);

    const shallow = await scan(
      `type Service struct { Store }
func (Service) DeleteInvoice(ctx context.Context, db *sql.DB, invoiceID string) error {
  _, err := db.ExecContext(ctx, "DELETE FROM invoices WHERE id = ? AND account_id = ?", invoiceID, "owned")
  return err
}`,
      "  service := Service{}",
    );
    expect(shallow).toHaveLength(1);
    expect(shallow[0]?.line).toBeGreaterThan(10);
    expect(
      shallow[0]?.frameworkModel?.propagators.some(
        ({ kind }) => kind === "go-method-receiver-promoted-field",
      ),
    ).toBe(false);
  });

  test("rejects ambiguous, cyclic, unresolved, and over-depth promoted methods", async () => {
    const inventory = async (declarations: string): Promise<ModelRecord[]> =>
      repositoryInventory({
        "invoices.go": `package invoices
import (
  "context"
...
func Handler(db *sql.DB, w http.ResponseWriter, r *http.Request) {
  ...
  service.DeleteInvoice(r.Context(), db, invoiceID)
    ...
    expect(
      await inventory(`type Left struct { Store }
type Right struct { Store }
type Service struct { Left; Right }`),
    ).toEqual([]);
    expect(
      await inventory(`type Left struct { Store }
type Service struct { Left; DeleteInvoice int }`),
    ).toEqual([]);
    expect(
      await inventory(`type CycleA struct { *CycleB }
type CycleB struct { *CycleA; *Store }
type Service struct { CycleA }`),
    ).toEqual([]);

    const depthSource = (depth: number): string => {
      if (depth === 1) return "type Service struct { Store }";
      const layers = ["type Layer0 struct { Store }"];
      for (let index = 1; index < depth - 1; index += 1)
        layers.push(`type Layer${index} struct { Layer${index - 1} }`);
      layers.push(`type Service struct { Layer${depth - 2} }`);
      return layers.join("\n");
    };
    expect(await inventory(depthSource(8))).toHaveLength(1);
    expect(await inventory(depthSource(9))).toEqual([]);

    expect(
      await repositoryInventory({
        "go.mod": `module example.com/promoted-unresolved
go 1.26`,
        "handler.go": `package invoices
import (
  "context"
...
type Local struct { Store }
type Service struct { Local; external.Unknown }
func Handler(db *sql.DB, w http.ResponseWriter, r *http.Request) {
  invoiceID := r.PathValue("invoiceID")
  service := &Service{}
  service.DeleteInvoice(r.Context(), db, invoiceID)
  ...
  test("rejects unbound, method-value, dynamic, and ambiguous method dispatch", async () => {
    const sink = `package invoices
